...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:param name="CCSData" select="$RtnDoc/QlfyReforestationCostsStmt"/>
    <!-- Common -Qualifying Reforestation Costs Statement-->  
   ...
   <table id="QRCTbl" class="styDepTblLandscape" style="text-align:center;">
     ...
     <tfoot/>
     <tbody>    
       <xsl:for-each select="$CCSData/QualifyingReforestationCostGrp">  
          <tr>
            ...
            <td class="styDepTblCell" style="width:93mm;text-align:left;">
              <xsl:call-template name="PopulateText">
                <xsl:with-param name="TargetNode" select="UniqueStrandIdentifierTxt"/>
              </xsl:call-template>
            </td>
            <td class="styDepTblCell" style="width:35mm;text-align:right;">
                <xsl:call-template name="PopulateText">
                  <xsl:with-param name="TargetNode" select="AcresReforestedNum"/>
                </xsl:call-template>
            </td>
            <td class="styDepTblCell" style="width:93mm;text-align:left;">
                <xsl:call-template name="PopulateText">
                  <xsl:with-param name="TargetNode" select="NatureReforestationTrtmntTxt"/>
                </xsl:call-template>
            </td>
            <td class="styDepTblCell" style="width:35mm;text-align:right;">
              <xsl:call-template name="PopulateAmount">
                <xsl:with-param name="TargetNode" select="QualifiedReforestationExpnsAmt"/>
              </xsl:call-template>
            </td>
          </tr>      
       </xsl:for-each>
 ...
 <xsl:text disable-output-escaping="yes">&lt;!DOCTYPE html&gt;</xsl:text>
     ...
     <head>
      ...
      <body class="styBodyClass">
        ...
        <xsl:call-template name="PopulateDepCommonLeftoverLandscape"><xsl:with-param name="TargetNode" select="$CCSData"/>
        </xsl:call-template>        
