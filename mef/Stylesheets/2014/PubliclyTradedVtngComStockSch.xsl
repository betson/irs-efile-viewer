            ...
            <xsl:if test="count($DependencyData/PubliclyTradedVtngComStockInfo)=0 or count($DependencyData/PubliclyTradedVtngComStockInfo)=1">
              <td class="styDepTblCell" scope="col" style="text-align:left;">
                  <xsl:call-template name="PopulateText">
                    <xsl:with-param name="TargetNode" select="$DependencyData/PubliclyTradedVtngComStockInfo/ClassOfVotingStockTxt"/>
                  </xsl:call-template>
                </td>
                <td class="styDepTblCell" scope="col" style="text-align:left;">
                  <xsl:call-template name="PopulateText">
                    <xsl:with-param name="TargetNode" select="$DependencyData/PubliclyTradedVtngComStockInfo/TradingSymbolDesc"/>
                  </xsl:call-template>
                </td>
                <td class="styDepTblCell" scope="col" style="text-align:center;">
                  <xsl:call-template name="PopulateText">
                    <xsl:with-param name="TargetNode" select="$DependencyData/PubliclyTradedVtngComStockInfo/CUSIPNum"/>
                  </xsl:call-template>
                </td>
            </xsl:if>
