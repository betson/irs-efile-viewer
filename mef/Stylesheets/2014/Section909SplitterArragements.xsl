 ...
 -->
  ...
  <xsl:template name="Section909SplitterArragementsTemp">
    <table id="Section909SplitterArragementsTbl" class="styDepTbl">
      ...
      <xsl:for-each select="$DependencyData/Sect909SplitterArrngmtsInfo">
        <tr>
...
<!--     <td class="styDepTblCell" style="text-align:center;" >
        ...
        <td class="styDepTblCell" style="width: 54mm;text-align:left;" >
            <xsl:call-template name="PopulateText">
              <xsl:with-param name="TargetNode" select="Section909SplitterArrangement" />
            </xsl:call-template>
        </td>
        <td class="styDepTblCell" style="width: 34mm;padding-left:10mm;" >
          <xsl:call-template name="PopulateAmount">
            <xsl:with-param name="TargetNode" select="TaxesPaid" />
          </xsl:call-template>
        </td>
        <td class="styDepTblCell" style="width: 34mm;padding-left:10mm;" >
          <xsl:call-template name="PopulateAmount">
            <xsl:with-param name="TargetNode" select="TaxesAccrued" />
          </xsl:call-template>        
        </td>
        <td class="styDepTblCell" style="width: 45mm;text-align:left;" >
            <xsl:call-template name="PopulateText">
              <xsl:with-param name="TargetNode" select="RelatedIncomeTakenIntoAccount" />
            </xsl:call-template>
        </td>
        </tr>
      </xsl:for-each>
