...
<!DOCTYPE xsl:stylesheet [<!ENTITY nbsp "&#160;">]><xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template name="ShowQZAB">
    ...
    <table id="QZABTbl" class="styDepTbl">
      ...
      <xsl:for-each select="$FormData/QZABCreditShareholdersSCorpGrp">
        <tr>
          ...
          <td class="styDepTblCell" style="text-align:right;width:94mm">
            <xsl:call-template name="PopulateAmount"><xsl:with-param name="TargetNode" select="ShareOfCreditAmt"/></xsl:call-template>             
          </td>                              
        </tr>
      </xsl:for-each>
    </table>
