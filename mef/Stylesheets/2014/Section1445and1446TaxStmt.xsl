      ...
      <xsl:for-each select="$DependencyData/ExcludableInfo">
        ...
        <td class="styDepTblCell" style="width: 93mm;text-align:right;">
            <xsl:call-template name="PopulateAmount">
              <xsl:with-param name="TargetNode" select="ExcludableAmt"/>
            </xsl:call-template>
        </td>
        <td class="styDepTblCell" style="width: 94mm; text-align:left;">
          <xsl:call-template name="PopulateText">
            <xsl:with-param name="TargetNode" select="ExcludableAmtExplanationTxt"/>
          </xsl:call-template>
        </td>        
        </tr>
      </xsl:for-each>
      ...
      <body class="styBodyClass">
        <xsl:call-template name="DocumentHeaderDependency"/>    
          ...
          <div class="styTopSectionLine" style="width:187mm;">
			    ...
			    <div style="float:left;clear:none;">
					<xsl:call-template name="PopulateAmount">
					  <xsl:with-param name="TargetNode" select="$DependencyData/EffectivelyConnectedIncmTaxAmt"/>
					</xsl:call-template>
			    </div>
        </div>
        <div class="styTopSectionLine" style="width:187mm;">
          <div style="float:left;clear:none;"><span class="styTopSectionLineLbl">Refigured Taxable Income:</span></div>
			  <div style="float:left;clear:none;">
					<xsl:call-template name="PopulateAmount">
					  <xsl:with-param name="TargetNode" select="$DependencyData/RefiguredTaxableIncomeAmt"/>
					</xsl:call-template>
			  </div>
        </div>
        ...
        <div class="styTopSectionLine" style="width:187mm;">
          <div style="float:left;clear:none;"><span class="styTopSectionLineLbl">Refigured Tax on ECI:</span></div>
			  <div style="float:left;clear:none;">
					<xsl:call-template name="PopulateAmount">
					  <xsl:with-param name="TargetNode" select="$DependencyData/RefiguredTaxAmt"/>
					</xsl:call-template>
			  </div>
        </div>  
        <div class="styTopSectionLine" style="width:187mm;">
          <div style="float:left;clear:none;"><span class="styTopSectionLineLbl">Calculated Section 1445 and 1446 Tax:</span></div>
			  <div style="float:left;clear:none;padding-top:2mm;">
					<xsl:call-template name="PopulateAmount">
					  <xsl:with-param name="TargetNode" select="$DependencyData/CalculatedTaxAmt"/>
					</xsl:call-template>
			  </div>
        </div>  
      </body>
