...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:param name="EOAIDSData" select="$RtnDoc/ExplnOfAddnlInterestDedStmt"/>
	<!-- Template to display Explanation of Additional Interest Deduction Statement-->
	<xsl:template name="ShowEOAIDS">
		<table id="EOAIDSTbl" class="styDepTbl" style="font-size:7pt;width:187mm;table-layout:fixed;">
			...
			<tfoot/>
			<tbody>
				<xsl:for-each select="$EOAIDSData/ExplnOfAddnlInterestDeduction">
					<tr>
	...
	<xsl:param name="depDocTitle">
		<xsl:call-template name="PopulateDisplayName">
			<xsl:with-param name="TargetNode" select="$EOAIDSData"/>
		</xsl:call-template>
	</xsl:param>
	<!-- Main template -->
	<xsl:template match="/">
		...
		<html>
			...
			<body class="styBodyClass">
				...
				<xsl:call-template name="ShowEOAIDS"/>
				<br/>
			</body>
		</html>
	</xsl:template>
