...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:param name="DependencyData" select="$RtnDoc/OthIncomeNonExemptExpendSch" />
  
  
  <!-- Template to display Itemized Other Income Schedule -->
  <xsl:template name="DependencyTemplate">
    <!-- Itemized Other Income Schedule Data -->
    <table id="ONEETbl" class="styDepTbl">
      ...
      <tfoot></tfoot>
      <tbody>
        <xsl:for-each select="$DependencyData/OthIncomeNonExemptExpenditure">
          <tr>
            ...
            <td class="styDepTblCell" style="text-align:left">
              <xsl:call-template name="PopulateText">
                <xsl:with-param name="TargetNode" select="TypeOfIncome" />
              </xsl:call-template>
            </td>
      ...
      <body class="styBodyClass">
        ...
        <div class="styDepTitleLine">
          <span class="styDepTitle">
            <span style="width:81mm;"><xsl:value-of select="$depDocTitle" /></span>        
          </span>
        </div>
