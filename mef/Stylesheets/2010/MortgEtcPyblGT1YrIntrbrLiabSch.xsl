...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <!--HINTS - Display name is Mortgages etc. Payable Greater Than 1 Year Interbranch Liabilities Schedule- 1120F -->
  <xsl:param name="depDocTitle">
    <xsl:call-template name="PopulateDisplayName">
      <xsl:with-param name="TargetNode" select="$DependencyData" />
    </xsl:call-template>
  </xsl:param>
  <xsl:template name="MortgEtcPyblGT1YrIntrbrLiabSchTemp">
  <table id="GeneralDependencyTbl" class="styDepTblLandscape">
    <tr class="styDepTblHdr">
      ...
      <xsl:choose>
        <xsl:when test="$DependencyData/MortgEtcPyblGT1YrIntrbrInfo/CorporationEIN">
        ...
        <xsl:otherwise>
          ...
          <th class="styDepTblCell" style="width:86mm;text-align:center;" scope="col">Mortgages etc. Payable Greater Than 1 Year Interbranch Liabilities Description</th>
         ...
         <!--Column 1 - Corporation Name -->
          <td class="styDepTblCell" style="width:85mm;text-align:left;vertical-align:middle;font-size:7pt">
            ...
            <xsl:attribute name="rowspan">
              <xsl:value-of select="count(MortgEtcPyblGT1YrIntrbrLnItem)"></xsl:value-of>
            </xsl:attribute>
            <!--Allow row to expand by column -->
          ...
          <xsl:if test="count(MortgEtcPyblGT1YrIntrbrLnItem)=0 or count (MortgEtcPyblGT1YrIntrbrLnItem)=1">
            <!--Column 3 - Description -->
            <td class="styDepTblCell" style="width:86mm;text-align:left;font-size:7pt">
              <xsl:call-template name="PopulateText">
                <xsl:with-param name="TargetNode" select="MortgEtcPyblGT1YrIntrbrLnItem/Description" />
              </xsl:call-template>
              <span class="styTablecellpad"/>
            </td>
            <!--Column 3 - Description -->
            ...
            <td class="styDepTblCell" style="width:30mm;text-align:right;font-size:7pt">
              <xsl:call-template name="PopulateAmount">
                 <xsl:with-param name="TargetNode" select="MortgEtcPyblGT1YrIntrbrLnItem/BeginingAmount" />
              </xsl:call-template>
              <span class="styTablecellpad"/>
            </td>
            <!--Column 4 - Beginning Amount -->
            ...
            <td class="styDepTblCell" style="width:30mm;text-align:right;font-size:7pt">
              <xsl:call-template name="PopulateAmount">
                <xsl:with-param name="TargetNode" select="MortgEtcPyblGT1YrIntrbrLnItem/EndingAmount" />
              </xsl:call-template>
              <span class="styTablecellpad"/>
            </td>
            <!--Column 5 - Ending Amount -->
          ...
          <xsl:if test="count(MortgEtcPyblGT1YrIntrbrLnItem)&gt;1">
  ...
  <xsl:template match="/">
    <html>
      ...
      <body class="styBodyClass">
        ...
        <xsl:call-template name="MortgEtcPyblGT1YrIntrbrLiabSchTemp" />
      </body>
    </html>
  </xsl:template>
</xsl:stylesheet>
