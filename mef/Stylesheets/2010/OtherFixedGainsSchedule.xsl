...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template name="OtherFixedGainsScheduleTemp">
    ...
    <div class="styTopSectionLine" style="width:187mm;">
      ...
      <div style="float:left;clear:none;width:118mm;">
        <xsl:call-template name="PopulateAmount">
          <xsl:with-param name="TargetNode" select="$DependencyData/TotalAmountOfTax"/>
        </xsl:call-template>
      </div>
    </div>
    <div class="styTopSectionLine" style="width:187mm;">
      ...
      <br />
        <xsl:call-template name="PopulateAmount">
          <xsl:with-param name="TargetNode" select="$DependencyData/AmountOfUSTaxPaid"/>
        </xsl:call-template>
      </div>
    </div>
    ...
    <table id="GeneralDependencyTbl" class="styDepTbl">
      ...
      <xsl:for-each select="$DependencyData/OtherFixedGainsInfo">
        <tr>
          ...
          <td class="styDepTblCell" style="width:37mm;text-align:right;font-size:7pt">
            <xsl:call-template name="PopulateAmount">
              <xsl:with-param name="TargetNode" select="AmountOfUSTaxPaid" />
            </xsl:call-template>
          </td>
        </tr>
      </xsl:for-each>
