...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <!--HINTS - Display name is  Itemized Other Current Non-US Assets Schedule - 1120F -->
  ...
  <table id="GeneralDependencyTbl" class="styDepTblLandscape">
    <tr class="styDepTblHdr">
      ...
      <xsl:choose>
        <xsl:when test="$DependencyData/ItmzOtherCurrentNonUSAst/CorporationEIN">
        ...
        <xsl:otherwise>
          ...
          <th class="styDepTblCell" style="width:86mm;text-align:center;" scope="col">Itemized Other Current Non-US Assets Description</th>
    ...
    <tfoot/>
    <tbody>
    <xsl:for-each select="$DependencyData/ItmzOtherCurrentNonUSAst">
      <tr>
         ...
         <!--Column 1 - Corporation Name -->
          <td class="styDepTblCell" style="width:85mm;text-align:left;vertical-align:middle;font-size:7pt">
            ...
            <xsl:attribute name="rowspan">
              <xsl:value-of select="count(ItmzOtherCurrNonUSAstLineItem)"></xsl:value-of>
            </xsl:attribute>
            <!--Allow row to expand by column -->
          ...
          <xsl:if test="count(ItmzOtherCurrNonUSAstLineItem)=0 or count (ItmzOtherCurrNonUSAstLineItem)=1">
            <!--Column 3 - Description -->
            <td class="styDepTblCell" style="width:86mm;text-align:left;font-size:7pt">
              <xsl:call-template name="PopulateText">
                <xsl:with-param name="TargetNode" select="ItmzOtherCurrNonUSAstLineItem/Description" />
              </xsl:call-template>
              <span class="styTablecellpad"/>
            </td>
            <!--Column 3 - Description -->
            ...
            <td class="styDepTblCell" style="width:30mm;text-align:right;font-size:7pt">
              <xsl:call-template name="PopulateAmount">
                 <xsl:with-param name="TargetNode" select="ItmzOtherCurrNonUSAstLineItem/BeginingAmount" />
              </xsl:call-template>
              <span class="styTablecellpad"/>
            </td>
            <!--Column 4 - Beginning Amount -->
            ...
            <td class="styDepTblCell" style="width:30mm;text-align:right;font-size:7pt">
              <xsl:call-template name="PopulateAmount">
                <xsl:with-param name="TargetNode" select="ItmzOtherCurrNonUSAstLineItem/EndingAmount" />
              </xsl:call-template>
              <span class="styTablecellpad"/>
            </td>
            <!--Column 5 - Ending Amount -->
          ...
          <xsl:if test="count(ItmzOtherCurrNonUSAstLineItem)&gt;1">
      ...
      <xsl:for-each select="ItmzOtherCurrNonUSAstLineItem">
