      ...
      <xsl:for-each select="$DependencyData/DedAllocAndApprtnToECI">
        <tr>
          ...
          <td class="styDepTblCell" style="text-align:left;">
            <xsl:choose>
              <xsl:when test="DeductionAllocToECILocationUS">
                <xsl:call-template name="PopulateUSAddressTemplate">
                  <xsl:with-param name="TargetNode" select="DeductionAllocToECILocationUS"/>
                </xsl:call-template>
              </xsl:when>
              <xsl:otherwise>
                <xsl:call-template name="PopulateForeignAddressTemplate">
                  <xsl:with-param name="TargetNode" select="DeductionAllocECILocationFrgn"/>
                </xsl:call-template>
              </xsl:otherwise>
            </xsl:choose>
          </td>
          ...
          <td class="styDepTblCell" style="text-align:right;">
            <xsl:call-template name="PopulateAmount">
              <xsl:with-param name="TargetNode" select="DeductionAllocToECIByLocation"/>
            </xsl:call-template>
          </td>
