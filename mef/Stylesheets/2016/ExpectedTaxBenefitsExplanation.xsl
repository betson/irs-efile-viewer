...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:param name="CCSData" select="$RtnDoc/ExpectedTaxBenefitsExplnTxt"/>
    ...
    <table id="ILSTbl" class="styDepTbl" style="text-align:center;">
      <thead class="styTableThead">
        <tr class="styDepTblHdr">          
           ...
           <th class="styDepTblCell" scope="col" style="width:86mm;">Expected Taxbenefits Explanation </th>        
        </tr>
      </thead>
        <tfoot/>
       <tbody>
       <!-- Table Name or First element name -->    
        <xsl:for-each select="$CCSData/ExpectedTaxBenefitsExpln">  
          <tr>
          ...
          <!-- 2nd Column -->
           ...
           <!-- </xsl:if>
            ...
            <xsl:call-template name="PopulateText">
                  <xsl:with-param name="TargetNode" select="ExpectedTaxBenefitsExplnTxt"/>
                </xsl:call-template>
            </td>
          </tr>      
        </xsl:for-each>
