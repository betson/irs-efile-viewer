...
<!DOCTYPE xsl:stylesheet [<!ENTITY nbsp "&#160;">]><xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template name="ShowDS">
    ...
    <xsl:for-each select="$Form990DepsData/NotePayableGrp">
      <table id="DSTbl" class="styDepTbl">           
        ...
        <tr class="styDepTblRow2">  
          ...
          <td class="styDepTblCell" style="text-align:left">     
            <xsl:choose>
              <xsl:when test="LenderNameGrp/PersonNm">
                <xsl:call-template name="PopulateText"><xsl:with-param name="TargetNode" select="LenderNameGrp/PersonNm"/></xsl:call-template>
              </xsl:when>
              <xsl:otherwise>
                <xsl:call-template name="PopulateText"><xsl:with-param name="TargetNode" select="LenderNameGrp/BusinessName/BusinessNameLine1Txt"/></xsl:call-template>
                <xsl:if test="LenderNameGrp/BusinessName/BusinessNameLine2Txt">
                  <br/><xsl:call-template name="PopulateText"><xsl:with-param name="TargetNode" select="LenderNameGrp/BusinessName/BusinessNameLine2Txt"/></xsl:call-template>
                </xsl:if>
              </xsl:otherwise>
            </xsl:choose>                
          </td>
      ...
      <head>
        ...
        <style type="text/css">
          <xsl:if test="not($Print) or $Print=''">
            <!--Generic CSS Styles located in the template called below -->
