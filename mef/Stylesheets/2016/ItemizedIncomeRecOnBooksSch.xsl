...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:param name="DependencyData" select="$RtnDoc/ItemizedIncomeRecOnBooksSch"/>
	 <!-- - Common Form displays as - Itemized Income Recorded on Books Schedule  - -->
  <xsl:param name="depDocTitle">
    <xsl:call-template name="PopulateDisplayName"><xsl:with-param name="TargetNode" select="$DependencyData"/>
    </xsl:call-template>  
  </xsl:param>  
  <xsl:template name="ItemizedIncomeRecOnBooksSchTemp">
    <table id="ItemizedIncomeRecOnBooksSchTbl" class="styDepTbl">
      <thead class="styTableThead">
        <tr class="styDepTblHdr">
           <th class="styDepTblCell" scope="col" style="width:48mm;">Corporation Name</th>
      ...
      <xsl:for-each select="$DependencyData/ItemizedIncomeRecOnBooksGrp">
        <tr>
    ...
    <xsl:text disable-output-escaping="yes">&lt;!DOCTYPE html&gt;</xsl:text>
      ...
      <body class="styBodyClass">
        <xsl:call-template name="DocumentHeaderDependency"/>          
          ...
          <xsl:call-template name="ItemizedIncomeRecOnBooksSchTemp"/>
      </body>
    </html>
  </xsl:template>
</xsl:stylesheet>