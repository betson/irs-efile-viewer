...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <!-- - 970 Form displays as - Credit Report Issued Statement--> 
  ...
  <xsl:template name="CreditReportIssuedStatementTemp">
    <table id="CreditReportIssuedStatementTbl" class="styDepTbl">
      ...
      <xsl:for-each select="$DependencyData/RecipientUsedGrp">
        <tr>
          ...
          <td class="styDepTblCell" style="text-align:left;width:45mm">
                <xsl:call-template name="PopulateText">
                  <xsl:with-param name="TargetNode" select="RecipientName/BusinessNameLine1Txt"/>
                </xsl:call-template>
                <xsl:if test="RecipientName/BusinessNameLine2Txt">             
                  <br/>
                   <xsl:call-template name="PopulateText">
                      <xsl:with-param name="TargetNode" select="RecipientName/BusinessNameLine2Txt"/>
                  </xsl:call-template>
              </xsl:if>
          </td> 
          <td class="styDepTblCell" style="text-align:center;width:35mm">
            <xsl:call-template name="PopulateText">
              <xsl:with-param name="TargetNode" select="IssuanceDt"/>
            </xsl:call-template>
          </td>          
          <td class="styDepTblCell" style="text-align:left;width:60mm">
            <xsl:call-template name="PopulateText">
              <xsl:with-param name="TargetNode" select="InventoryMethodTxt"/>
            </xsl:call-template>
          </td>        
        </tr>
      </xsl:for-each>
