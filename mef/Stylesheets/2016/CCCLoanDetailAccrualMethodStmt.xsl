...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"> 
  ...
  <xsl:param name="DependencyData" select="$RtnDoc/CCCLoanDetailAccrualMethodStmt"/>
  <!-- 1040 Schedule F displays as - Commodity Credit Corporation Loan Detail Accrual Method Statement -->
  <xsl:param name="depDocTitle">	 
  <xsl:call-template name="PopulateDisplayName"><xsl:with-param name="TargetNode" select="$DependencyData"/></xsl:call-template>  
  </xsl:param>
  <xsl:template name="CCCLoanDetailAccrualMthdStmtTemp">
    <table id="CCCLoanDetailAccrualMthdStmtTbl" class="styDepTbl">
  ...
  <xsl:template match="/">
    <xsl:text disable-output-escaping="yes">&lt;!DOCTYPE html&gt;</xsl:text>
      ...
      <xsl:call-template name="DocumentHeaderDependency"/>
        ...
        <xsl:call-template name="CCCLoanDetailAccrualMthdStmtTemp"/>    
      </body>
    </html>
  </xsl:template>
</xsl:stylesheet>