					...
					<tr>
								...
								<xsl:choose>
									<xsl:when test="SCorporationNameAndAddress/BusinessName">
										<xsl:call-template name="PopulateText">
											<xsl:with-param name="TargetNode" select="SCorporationNameAndAddress/BusinessName/BusinessNameLine1Txt"/>
										</xsl:call-template>
										<xsl:if test="normalize-space(SCorporationNameAndAddress/BusinessName/BusinessNameLine2Txt)!=''">
											<br/>
											<xsl:call-template name="PopulateText">
												<xsl:with-param name="TargetNode" select="SCorporationNameAndAddress/BusinessName/BusinessNameLine2Txt"/>
											</xsl:call-template>
										</xsl:if>
									</xsl:when>
									<xsl:otherwise>
										<xsl:call-template name="PopulateText">
											<xsl:with-param name="TargetNode" select="SCorporationNameAndAddress/PersonNm"/>
										</xsl:call-template>
									</xsl:otherwise>
								</xsl:choose>
							</span>
						...
						<td class="styDepTblCell" align="left">
							<span style="width:50mm">
								<xsl:choose>
									<xsl:when test="SCorporationNameAndAddress/USAddress">
										<xsl:call-template name="PopulateUSAddressTemplate">
											<xsl:with-param name="TargetNode" select="SCorporationNameAndAddress/USAddress"/>
										</xsl:call-template>
									</xsl:when>
									<xsl:otherwise>
										<xsl:call-template name="PopulateForeignAddressTemplate">
											<xsl:with-param name="TargetNode" select="SCorporationNameAndAddress/ForeignAddress"/>
										</xsl:call-template>
									</xsl:otherwise>
								</xsl:choose>
							</span>
