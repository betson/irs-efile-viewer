...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:param name="DependencyData" select="$RtnDoc/ItemizedExpensesRecOnBooksSch2"/>
  <!-- - Common Form displays as - Itemized Expenses Recorded on Books Schedule  -->
  <xsl:param name="depDocTitle">
    <xsl:call-template name="PopulateDisplayName"><xsl:with-param name="TargetNode" select="$DependencyData"/>
    </xsl:call-template>  
  </xsl:param>  
  <xsl:template name="ItemizedExpensesRecOnBooksSch2Temp">
    <table id="ItemizedExpensesRecOnBooksSch2Tbl" class="styDepTbl">
      ...
      <xsl:for-each select="$DependencyData/ItemizedExpensesRecOnBooks">
        <tr>
    ...
    <xsl:text disable-output-escaping="yes">&lt;!DOCTYPE html&gt;</xsl:text>
      ...
      <body class="styBodyClass" style="187mm;">
        ...
        <span class="styDepTitle"  style="padding-right:2mm;">
          ...
          <xsl:call-template name="ItemizedExpensesRecOnBooksSch2Temp"/>
      </body>
    </html>
  </xsl:template>
</xsl:stylesheet>