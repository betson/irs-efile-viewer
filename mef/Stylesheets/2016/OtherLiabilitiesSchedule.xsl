...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:param name="DependencyData" select="$RtnDoc/OtherLiabilitiesSchedule"/>
  ...
  <xsl:template name="ShowDependencyData">
    ...
    <table id="DependencyDataTbl" class="styDepTbl">
      ...
      <tr class="styDepTblHdr">         
         ...
         <th class="styDepTblCell" scope="col">Beginning of Year - Book Value</th>       
       ...
       <tbody>
      <xsl:for-each select="$DependencyData/OtherLiabilitiesDetail">
        <tr>
          ...
          <td class="styDepTblCell" style="text-align:right; width: 35mm">
            <xsl:call-template name="PopulateAmount">
              <xsl:with-param name="TargetNode" select="BOYBookValueAmt"/>
            </xsl:call-template>
          </td>               
           <td class="styDepTblCell" style="text-align:right; width: 35mm">
            <xsl:call-template name="PopulateAmount">
  ...
  <xsl:template match="/">
    <xsl:text disable-output-escaping="yes">&lt;!DOCTYPE html&gt;</xsl:text>
      ...
      <!-- NameDisplay - IRS990PF - Other Liabilities Schedule -->
