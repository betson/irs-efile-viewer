...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template name="DependencyTemplate1">
  ...
  <xsl:template name="DependencyTemplate2">
   <table class="styDepTblLandscape" style="margin-top:4mm; font-size:6pt; font-family:Verdana">
      <thead class="styTableThead">
        <tr class="styDepTblHdr">
		  <th class="styDepTblCell" scope="col" colspan="2">
  ...
  <xsl:template name="DependencyTemplate3">
   <table class="styDepTblLandscape" style="margin-top:4mm; font-size:6pt; font-family:Verdana">
      <thead class="styTableThead">
        <tr class="styDepTblHdr">
          <th class="styDepTblCell" scope="col" colspan="2">
      ...
      <xsl:for-each select="$DependencyData/CdSectPropDeprecAmortzInfoTxt">  
        <tr>
          ...
          <xsl:choose>
           ...
           <td class="styDepTblCell"  style="text-align:left;"> 
            <xsl:call-template name="PopulateText">
              <xsl:with-param name="TargetNode" select="PresentMethodGrp/SpclDeprecAllowanceExplnTxt"/>
            </xsl:call-template>      
          </td>    
          <td class="styDepTblCell"  style="text-align:left;">   
            <xsl:call-template name="PopulateText">
              <xsl:with-param name="TargetNode" select="ProposedMethodGrp/SpclDeprecAllowanceExplnTxt"/>
            </xsl:call-template> 
          </td>
           <td class="styDepTblCell"  style="text-align:left;"> 
            <xsl:call-template name="PopulateText">
              <xsl:with-param name="TargetNode" select="PresentMethodGrp/SingleMultGeneralAssetAcctTxt"/>
            </xsl:call-template>      
          </td>    
          <td class="styDepTblCell"  style="text-align:left;">   
            <xsl:call-template name="PopulateText">
              <xsl:with-param name="TargetNode" select="ProposedMethodGrp/SingleMultGeneralAssetAcctTxt"/>
            </xsl:call-template> 
          </td>
        </tr>
      </xsl:for-each>
  ...
  <xsl:template match="/">
    <xsl:text disable-output-escaping="yes">&lt;!DOCTYPE html&gt;</xsl:text>
      ...
      <body class="styBodyClass">   
        ...
        <xsl:call-template name="DependencyTemplate1"/> 
        <xsl:call-template name="DependencyTemplate2"/>
        <xsl:call-template name="DependencyTemplate3"/>    
      </body>
    </html>
  </xsl:template>
</xsl:stylesheet>