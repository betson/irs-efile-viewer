...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:param name="RootElt" select="$RtnDoc/RecaptureStatement"/>
  
  <!-- Template to display Recapture Statement -->
  
  <xsl:template name="ShowData">
    <table id="ILSTbl" class="styDepTbl" style="text-align:center;">
      <thead class="styTableThead">
        <tr class="styDepTblHdr">          
           <th class="styDepTblCell" scope="col" style="width:93mm;">Recapture Literal</th>        
           <th class="styDepTblCell" scope="col" style="width:93mm;">Recapture Amount</th>        
        </tr>
      </thead>
       <tbody>    
        <xsl:for-each select="$RootElt/RecaptureStmt">  
          <tr>
            ...
            <td class="styDepTblCell" style="text-align:left;">
              <xsl:call-template name="PopulateText">
                <xsl:with-param name="TargetNode" select="RecaptureLiteralCd"/>
              </xsl:call-template>
            </td>
            <td class="styDepTblCell" style="text-align:right;">
              <xsl:call-template name="PopulateAmount">
                <xsl:with-param name="TargetNode" select="RecaptureAmt"/>
              </xsl:call-template>
            </td>
          </tr>
        </xsl:for-each>
