...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:param name="DependencyData" select="$RtnDoc/OtherIncomeSchedule3" />
	...
	<xsl:template name="ShowDependencyData">
		...
		<table id="INOLDTbl" class="styDepTbl" style="font-size:7pt;">
			<thead class="styTableThead">
				<tr class="styDepTblHdr">           
					...
					<th class="styDepTblCell" scope="col" style="width:107mm;">Other income description</th>  
					<th class="styDepTblCell" scope="col" style="width:30mm;">Other income amount</th>              
				</tr>
			</thead>
			<tbody>
				<xsl:for-each select="$DependencyData/OtherIncomeSchedule3Grp">
					<tr>
		...
		<div class="styTopSectionLine">
			...
			<div class="styExplanationLine" style="float:left">
				<xsl:call-template name="PopulateAmount">
					<xsl:with-param name="TargetNode" select="$DependencyData/TotalOtherIncomeAmt"/>
				</xsl:call-template>
			</div>
		</div>
	</xsl:template>
