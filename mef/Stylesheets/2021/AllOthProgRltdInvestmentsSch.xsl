...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:param name="DependencyData" select="$RtnDoc/AllOthProgRltdInvestmentsSch"/>
  ...
  <xsl:template match="/">
    <xsl:text disable-output-escaping="yes">&lt;!DOCTYPE html&gt;</xsl:text>
     ...
     <!-- NameDisplay - IRS990PF - All Other Program Related Investments Schedule -->
      <body class="styBodyClass">      
        ...
        <xsl:call-template name="AllOthProgRltdInvestmentsSchTemp"/>
      </body>
    </html>
  </xsl:template>
</xsl:stylesheet>