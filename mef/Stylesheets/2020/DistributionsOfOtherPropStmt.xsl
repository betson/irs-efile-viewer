...
<!-- Created by Doug Peterson 6-15-2006--><xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template name="ShowAOVSS">
    <table id="AOVSSTbl" class="styDepTbl">
      ...
      <tfoot/>
      <tbody>    
        <xsl:for-each select="$AOVSSData/DistributionsOfOtherPropGrp">  
          <tr>
            ...
            <td class="styDepTblCell" style="text-align:right">
              <xsl:call-template name="PopulateAmount">
                <xsl:with-param name="TargetNode" select="AdjustedBasisOfPropertyAmt"/>
              </xsl:call-template>
            </td>  
            <!-- Second Column -->
            <td class="styDepTblCell" style="text-align:right;">
              <xsl:call-template name="PopulateAmount">
                <xsl:with-param name="TargetNode" select="PropertyFMVAmt"/>
              </xsl:call-template>
            </td>       
          </tr>      
        </xsl:for-each>
