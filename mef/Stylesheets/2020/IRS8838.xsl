...
<body class="styBodyClass" style="width:187mm">
  ...
  <div class="styBB" style="width:187mm;height:auto;">
    <div class="styFNBox" style="width:28mm;font-size: 7pt;height:18mm;">
     ...
     <div class="styFST" style="height:5mm;margin-left:3mm;font-weight:bold;padding-top:2mm;">
      ...
      <div class="styFST" style="height:5mm;font-size:6pt;">
				<img src="{$ImagePath}/8838_Bullet_Sm.gif" alt="bullet image"/>
				 ...
				 <a style="text-decoration:none;color:black;" href="http://www.irs.gov/form8838." title="Link to irs.gov">
    ...
    <div class="styTYBox" style="width:37mm; height: 9mm;padding-top:2mm; font-size:7pt; border-bottom-width:1px">OMB No. 1545-1395
     ...
     <div class="styTYBox" style="width:37mm; height:9mm;padding-top:3mm; font-size:7pt; padding-left:4mm;text-align:left;">Attachment<br/>
  ...
  <div class="styBB" style="width:187mm">
      ...
      <div style="font-size: 7pt;font-family:verdana;">
        <xsl:choose>
          <xsl:when test="normalize-space($FormData/TaxpayerName) != ''">
            <xsl:call-template name="PopulateText"><xsl:with-param name="TargetNode" select="$FormData/TaxpayerName/BusinessNameLine1Txt"/></xsl:call-template>
            <xsl:if test="normalize-space($FormData/TaxpayerName/BusinessNameLine2Txt)!=''">
              <br/>
              <xsl:call-template name="PopulateText"><xsl:with-param name="TargetNode" select="$FormData/TaxpayerName/BusinessNameLine2Txt"/></xsl:call-template>
            </xsl:if>            
          </xsl:when>  
          <xsl:when test="normalize-space($FormData/TaxpayerNm) != ''">
            <xsl:call-template name="PopulateText"><xsl:with-param name="TargetNode" select="$FormData/TaxpayerNm"/></xsl:call-template>
          </xsl:when>  
         <xsl:otherwise>
    ...
    <div class="styEINBox" style="width: 62mm; padding-left:2mm; height: 9mm; font-size: 7pt;font-weight:normal;font-weight:bold;">Identifying number (see instructions)
      <div style="text-align:left;font-weight:bold;">  
        <br/>  
					 <xsl:if test="$FormData/SSN != ' ' ">
    ...
    <div class="styEINBox" style="width: 62mm; padding-left:2mm; height: 9mm; border-color: black; border-style: solid; border-bottom-width: 0px; border-left-width: 0px; border-right-width: 0px; border-top-width: 1px; font-weight:normal;font-size: 7pt;">Social security number of spouse (only if a joint income tax return was filed))<br/>
			<span style="width:100%;text-align:left;padding-top:2mm">
				<xsl:if test="$FormData/SpouseSSN != ' ' ">
					<xsl:call-template name="PopulateSSN">
							 <xsl:with-param name="TargetNode" select="$FormData/SpouseSSN"/>
					</xsl:call-template>	
				</xsl:if>
			</span>
		</div>
  ...
  <div class="styBB" style="width: 187mm; height: 10mm">Number, street, and room or suite no. (If a P.O. box, see instructions.)<br/>
		<xsl:choose>
			...
			<xsl:when test="  $FormData/USAddress/AddressLine1Txt!=''">
				<xsl:call-template name="PopulateText">
					<xsl:with-param name="TargetNode" select="$FormData/USAddress/AddressLine1Txt"/>
				</xsl:call-template>
				<br/>
				<xsl:if test="normalize-space($FormData/USAddress/AddressLine1Txt)!=''">
					<xsl:call-template name="PopulateText">
						<xsl:with-param name="TargetNode" select="$FormData/USAddress/AddressLine2Txt"/>
					</xsl:call-template>
					<br/>
				</xsl:if>
			</xsl:when>
  ...
  <div class="styBB" style="width: 187mm; height: 10mm">City or town, state, and ZIP code<br/>
		<xsl:choose>
			<xsl:when test="  $FormData/USAddress/CityNm!=''">
				<xsl:call-template name="PopulateText">
					<xsl:with-param name="TargetNode" select="  $FormData/USAddress/CityNm"/>
				</xsl:call-template>
				<span class="styTableCellPad"/>
				<xsl:call-template name="PopulateText">
					<xsl:with-param name="TargetNode" select="  $FormData/USAddress/StateAbbreviationCd"/>
				</xsl:call-template>
				<span class="styTableCellPad"/>
				<xsl:call-template name="PopulateText">
					<xsl:with-param name="TargetNode" select="  $FormData/USAddress/ZIPCd"/>
				</xsl:call-template>
				<span class="styTableCellPad"/>
			</xsl:when>
			<xsl:otherwise>
  ...
  <div style="width: 187mm;font-size:7pt;">The taxpayer(s) listed above and the Commissioner of Internal Revenue, pursuant to the regulations under section 367, agree to the following:</div>
  
  <!--  Line 1 -->    
  <div style="width: 187mm">  
    <div class="styLNLeftNumBox" style="padding-left: 2mm">1</div>
    <span class="styLNDesc" style="width: 177mm;font-size:7pt;">
      The amount of any federal income tax due relating to the transfer described in item 5, below, on any income tax return made by or for the 
    </span>      
  </div>
  <div style="width: 187mm">  
    <span class="styLNDesc" style="padding-left: 8mm; width: 177mm;font-size:7pt;height:auto;">
    above taxpayer(s) for the tax year ended 
      <xsl:choose>
        <xsl:when test="$FormData/TaxYearEndDt != ''">
      ...
      <div style="width: 187mm"><span style="width: 56mm"/>month<span style="width: 11mm"/>day<span style="width:13mm"/>year</div>
    </span>      
  </div>
  <div style="width: 187mm">  
    <span class="styLNDesc" style="padding-left: 8mm; width: 177mm;height:auto;">
      ...
      <div style="width: 187mm"><span style="width: 4mm"/>month<span style="width: 11mm"/>day<span style="width: 13mm"/>year</div>
    </span>      
  </div>
  <!--  Line 1 -->
  ...
  <div style="width: 187mm">  
    ...
    <div class="styBB" style="float: right; clear: none; border-style: solid; width: 113mm; border-bottom-width: 1px;font-family:verdana;">
      <xsl:call-template name="PopulateText"><xsl:with-param name="TargetNode" select="$FormData/TransfereeName/BusinessNameLine1Txt"/></xsl:call-template>
    </div>    
  </div>    
  <div style="width: 187mm">
    <div class="styBB" style="float: right; border-style: solid; width: 178mm;">
      <xsl:call-template name="PopulateText"><xsl:with-param name="TargetNode" select="$FormData/TransfereeName/BusinessNameLine2Txt"/></xsl:call-template>
