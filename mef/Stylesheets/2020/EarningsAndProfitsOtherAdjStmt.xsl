...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:param name="DependencyData" select="$RtnDoc/EarningsAndProfitsOtherAdjStmt"/>
  ...
  <xsl:template name="DependencyTemplate">
    ...
    <table id="EPOASTbl" class="styDepTbl" style="margin-top:4mm; font-size:8pt; font-family:Arial">
      ...
      <tfoot/>
      <tbody>  
      <xsl:for-each select="$DependencyData/ItemizedErngsPrfOtAdjStmt">
        <tr>
          ...
          <td class="styDepTblCell" align="right" nowrap="nowrap">
			 <xsl:call-template name="PopulateAmount"><xsl:with-param name="TargetNode" select="ForeignAmt" /></xsl:call-template>
		  </td>          
        </tr>
      </xsl:for-each>
      </tbody>    
      ...
      <body class="styBodyClass">    
        ...
        <div class="styDepTitleLine">
          <div class="styDepTitle" style="width:82mm">
            <xsl:value-of select="$depDocTitle"/>
            <!--TY 2003 Earnings and Profits Other Adjustments Statement-->
          </div>
        </div>        
