...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:param name="DependencyData" select="$RtnDoc/AdvertisingIncomeCnsldtSch" />
	...
	<xsl:template name="ShowDependencyData">
		...
		<table id="INOLDTbl" class="styDepTbl" style="font-size:7pt;">
			...
			<tbody>
				<xsl:for-each select="$DependencyData/GrossAdvertisingIncmGrp">
					<tr>
		...
		<div class="styTopSectionLine">
			<div class="styTopSectionLineLbl" style="float:left">
				Total gross advertising income:
			</div>
			<div class="styExplanationLine" style="float:left">
				<xsl:call-template name="PopulateAmount">
					<xsl:with-param name="TargetNode" select="$DependencyData/TotalGrossAdvertisingIncomeAmt"/>
				</xsl:call-template>
			</div>
		</div>
		<div class="styStdDiv" style="font-weight:bold;padding-top:2mm;font-size:8pt;">
		...
		<table id="INOLDTbl" class="styDepTbl" style="font-size:7pt;">
			...
			<tbody>
				<xsl:for-each select="$DependencyData/DirectAdvertisingCostGrp">
					<tr>
						<xsl:attribute name="class">
							<xsl:choose>
								<xsl:when test="position() mod 2 = 1">styDepTblRow1</xsl:when>
		...
		<div class="styTopSectionLine">
			...
			<div class="styExplanationLine" style="float:left">
				<xsl:call-template name="PopulateAmount">
					<xsl:with-param name="TargetNode" select="$DependencyData/TotalDirectAdvertisingCostAmt"/>
				</xsl:call-template>
			</div>
		</div>
		<div class="styTopSectionLine">
			<div class="styTopSectionLineLbl" style="float:left">
				Advertising gain or loss:
			</div>
			<div class="styExplanationLine" style="float:left">
				<xsl:call-template name="PopulateAmount">
					<xsl:with-param name="TargetNode" select="$DependencyData/AdvertisingGainLossAmt"/>
				</xsl:call-template>
			</div>
		</div>
	</xsl:template>
