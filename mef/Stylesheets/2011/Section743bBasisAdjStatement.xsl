...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template name="ShowAOVSS">
    <table id="AOVSSTbl" class="styDepTbl">
      ...
      <tfoot></tfoot>
      <tbody>    
        <xsl:for-each select="$AOVSSData/Section743bBasisAdjustment">  
          <tr>
            ...
            <td class="styDepTblCell" style="text-align:left;">
              <xsl:call-template name="PopulateText">
                ...
                <xsl:with-param name="TargetNode" select="NameOfTheTransfereePartner/BusinessNameLine1" />
              </xsl:call-template>
              <br/>
              <xsl:call-template name="PopulateText">
                <xsl:with-param name="MaxSize" select="3" />
                <xsl:with-param name="TargetNode" select="NameOfTheTransfereePartner/BusinessNameLine2" />
              </xsl:call-template>
            ...
            <td class="styDepTblCell" style="text-align:center;width:33mm;">
               ...
               <xsl:if test="$AOVSSData/Section743bBasisAdjustment/SSNAmount != '' ">
                <xsl:call-template name="PopulateSSN">
                	<xsl:with-param name="TargetNode" select="SSNAmount" />
                	 </xsl:call-template>
                </xsl:if>
                <xsl:if test="$AOVSSData/Section743bBasisAdjustment/EINAmount != '' ">
                <xsl:call-template name="PopulateEIN">
                <xsl:with-param name="TargetNode" select="EINAmount" />
                </xsl:call-template>
                </xsl:if>
            ...
            <td class="styDepTblCell" style="text-align:left;">
              <xsl:call-template name="PopulateText">
                <xsl:with-param name="TargetNode" select="PartnershipProperties" />
              </xsl:call-template>
            </td>        
          </tr>      
        </xsl:for-each>
