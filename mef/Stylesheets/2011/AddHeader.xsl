...
<xsl:stylesheet version="1.0"
	...
	<xsl:template name="DocumentHeaderContent">
		...
		<span class="styDoNotProcess">
			...
			<xsl:choose>
				<xsl:when
					test="$Parameters/SubmissionType='1040PR' and starts-with($Parameters/DocumentName,'IRS1040SSPR') and $Parameters/Language='SPANISH'">
					<span>
						<span style="width:70px;" />
						<image name="EnglishBtn" id="EnglishBtn"
							onclick="top.ReturnTree.LabelClick( 'IRS1040SSPR', '', 'IRS1040SSPRENGLISH', 'getSubDoc' );"
							style="vertical-align:top;cursor:hand;" src="{$NonVersionedImagePath}/refreshENGButton.gif"
							alt="Select to display in English" />
					</span>
				</xsl:when>
				<xsl:when
					test="$Parameters/SubmissionType='1040PR' and starts-with($Parameters/DocumentName,'IRS1040SSPR') and $Parameters/Language='ENGLISH'">
					<span>
						<span style="width:70px;" />
						<image name="SpanishBtn" id="SpanishBtn"
							onclick="top.ReturnTree.LabelClick( 'IRS1040SSPR', '', 'IRS1040SSPRSPANISH', 'getSubDoc' );"
							style="vertical-align:top;cursor:hand;" src="{$NonVersionedImagePath}/refreshSPButton.gif"
							alt="Select to display in Spanish" />
					</span>
				</xsl:when>
			</xsl:choose>
