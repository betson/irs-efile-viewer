...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template name="ShowATI">
    ...
    <table id="ATITbl" class="styDepTbl">
      ...
      <xsl:for-each select="$FormData/AdjustmentToTaxableIncome">
        <tr>
          ...
          <td class="styDepTblCell" style="text-align:center;width:35mm">            
            <xsl:call-template name="PopulateText"><xsl:with-param name="TargetNode" select="RecomputationYear" /></xsl:call-template>             
          </td>                      
          <td class="styDepTblCell" style="text-align:left;width:102mm ">
            <xsl:if test="NameOfEntity/BusinessNameLine1 !=''">
          ...
          <td class="styDepTblCell" style="text-align:right;width:50mm">
            <xsl:call-template name="PopulateAmount"><xsl:with-param name="TargetNode" select="AmountOfNetChangeToDeprec" /></xsl:call-template>             
          </td>                              
        </tr>
      </xsl:for-each>
    </table>
