...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:param name="CCSData" select="$RtnDoc/PremiumIncomeSchedule" />
  
  <!-- Template to display Premium Income Schedule-->  
  ...
  <xsl:template name="ShowPIS">
    <table id="PISTbl" class="styDepTblLandscape" style="text-align:center;">
      <thead class="styTableThead">
       ...
       <tbody>    
        <xsl:for-each select="$CCSData/PremiumIncome">  
          <tr>
            ...
            <td class="styDepTblCell" style="text-align:right;">
                <xsl:call-template name="PopulateAmount">
                  <xsl:with-param name="TargetNode" select="IncomePerTaxReturnAmount" />
                </xsl:call-template>
            </td>
          </tr>      
        </xsl:for-each>
