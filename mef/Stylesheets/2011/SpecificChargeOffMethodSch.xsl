    ...
    <table id="SpecificChargeOffMethodSch" class="styDepTbl">
      <thead class="styTableThead">
        <tr class="styDepTblHdr">
             ...
             padding-left:25mm">Name of Debtor</th>
             <th class="styDepTblCell"  scope="col" style="width:45mm;
             padding-left:25mm">Amount of Bad Debt Deduction</th>         
        </tr>
      </thead>
      <xsl:for-each select="$DependencyData/SpecificChargeOffMethod">
        <tr>
          ...
          <xsl:choose>
            ...
            <xsl:choose>
             <xsl:when test="BusinessNameOfDebtor != ''">
              <xsl:call-template name="PopulateText">
              <xsl:with-param name="TargetNode" select="BusinessNameOfDebtor/BusinessNameLine1" /> 
                         </xsl:call-template>
                          <span style="width:1px;"/><br/>
                          <xsl:call-template name="PopulateText">
              <xsl:with-param name="TargetNode" select="BusinessNameOfDebtor/BusinessNameLine2" /> 
                         </xsl:call-template>
                          <span style="width:1px;"/>
             </xsl:when>
             <xsl:otherwise>
              <xsl:call-template name="PopulateText">
              <xsl:with-param name="TargetNode" select="PersonNameOfDebtor" /> 
               </xsl:call-template>
                </xsl:otherwise>
                </xsl:choose>
               </td>                          
              ...
              <xsl:call-template name="PopulateAmount">
                <xsl:with-param name="TargetNode" select="BadDebtDeduction" />
              </xsl:call-template>
            </td>
           </tr>
      </xsl:for-each>
