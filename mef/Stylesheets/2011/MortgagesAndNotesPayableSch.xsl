...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:param name="Form990DepsData" select="$RtnDoc/MortgagesAndNotesPayableSch" />
  ...
  <xsl:template name="ShowDS">
    ...
    <xsl:for-each select="$Form990DepsData/NotePayable">
      <table id="DSTbl" class="styDepTbl">           
        ...
        <tr class="styDepTblRow2">  
          ...
          <td class="styDepTblCell" style="text-align:left">     
            <xsl:choose>
              <xsl:when test="LendersName/Individual">
                <xsl:call-template name="PopulateText"><xsl:with-param name="TargetNode" select="LendersName/Individual" /></xsl:call-template>
              </xsl:when>
              <xsl:otherwise>
                <xsl:call-template name="PopulateText"><xsl:with-param name="TargetNode" select="LendersName/Business/BusinessNameLine1" /></xsl:call-template>
                <xsl:if test="LendersName/Business/BusinessNameLine2">
                  <br /><xsl:call-template name="PopulateText"><xsl:with-param name="TargetNode" select="LendersName/Business/BusinessNameLine2" /></xsl:call-template>
                </xsl:if>
              </xsl:otherwise>
            </xsl:choose>                
          </td>
        ...
        <tr class="styDepTblRow2">  
          <th class="styDepTblCell" scope="col" nowrap="nowrap" style="text-align: right; width: 45%">Relationship to Insider</th>                                
          <td class="styDepTblCell" style="text-align:left">
      ...
      <body class="styBodyClass">
        ...
        <div class="styTopSectionLine">
          ...
          <span style="font-size:10pt;">
            <xsl:call-template name="PopulateText"><xsl:with-param name="TargetNode" select="$Form990DepsData/TotalMortgageAmount" /></xsl:call-template>
          </span>  
        </div>      
        <br/>            
