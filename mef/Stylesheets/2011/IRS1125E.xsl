		  ...
		  <form name="Form1125E">
			...
			<div style="width:187mm;" class="styTBB">
			  ...
			  <div style="width:110mm;height:23.5mm;" class="styFTBox">
				...
				<div style="height:3.5mm;font-size:7pt;padding-top:.5mm;border:0px;" class="styFST">
                  ...
                  Attach to Form 1120, 1120-C, 1120-F, and 1120-RIC
				</div>
			...
			<div style="width:187mm;" class="styBB">
			  <div style="height:4mm;padding-top:2mm;">
				<b>Note. </b>
				Complete Schedule E only if total receipts are $500,000 or more.   See instructions for definition of total receipts.
			  </div>
				...
				<xsl:choose>
				  <xsl:when test="$Form1125EData/Officer">
					<tbody style="border-color:black;">
					  <xsl:if test="($Print != $Separated) or (($Print=$Separated) and (count($Form1125EData/Officer) &lt; 5))">
						<xsl:for-each select="$Form1125EData/Officer">
						  <tr style="vertical-align:bottom;font-size: 7pt; font-weight: normal;">
							<td style="width:50mm;text-align:left;" class="styTableCell">
							  <xsl:if test="position() = last() and position() &gt;= 5">
					  ...
					  <xsl:if test="(count($Form1125EData/Officer)&lt; 1) or (($Print=$Separated) and (count($Form1125EData/Officer) &gt;5))">
						<xsl:call-template name="Populateirs1125EemptyCells">
						  <xsl:with-param name="index">1</xsl:with-param>
						</xsl:call-template>
					  </xsl:if>
					  <xsl:if test="(count($Form1125EData/Officer)&lt; 2) or (($Print=$Separated) and (count($Form1125EData/Officer) &gt;5))">
						<xsl:call-template name="Populateirs1125EemptyCells">
						  <xsl:with-param name="index"/>
						</xsl:call-template>
					  </xsl:if>
					  <xsl:if test="(count($Form1125EData/Officer)&lt; 3) or (($Print=$Separated) and (count($Form1125EData/Officer) &gt;5))">
						<xsl:call-template name="Populateirs1125EemptyCells">
						  <xsl:with-param name="index"/>
						</xsl:call-template>
					  </xsl:if>
					  <xsl:if test="(count($Form1125EData/Officer)&lt; 4) or (($Print=$Separated) and (count($Form1125EData/Officer) &gt;5))">
						<xsl:call-template name="Populateirs1125EemptyCells">
						  <xsl:with-param name="index"/>
						</xsl:call-template>
					  </xsl:if>
					  <xsl:if test="(count($Form1125EData/Officer)&lt; 5) or (($Print=$Separated) and (count($Form1125EData/Officer) &gt;5))">
						<xsl:call-template name="Populateirs1125EemptyCells">
						  <xsl:with-param name="index"/>
			...
			<div style="width:187mm;clear:both;">
			  ...
			  <div style="float:right;" class="styGenericDiv">
				<span class="styBoldText">1125-E</span> (12-2011)</div>
			  </div>
			  <p class="pageend"/>
			  <!-- *********************************************************** -->
			  ...
			  <xsl:if test="($Print = $Separated) and (count($Form1125EData/Officer) &gt;5)">
				<br/>
				  ...
				  <tbody style="border-color:black;">
					<xsl:for-each select="$Form1125EData/Officer">
					  <tr>
						...
						<td style="width:21mm;border-right-width:1px;" class="styDepTblCell">
						  <xsl:if test="position() = last() and position() &gt;= 5">
							<xsl:attribute name="style">border-right-width:1px;border-bottom-width:0px;</xsl:attribute>
						  </xsl:if>
						  <xsl:call-template name="PopulatePercent">
							<xsl:with-param name="TargetNode" select="PercentCommonStockOwned"/>
						  </xsl:call-template>
						...
						<td style="width:21mm;border-left-width:0px;" class="styDepTblCell">
						  <xsl:if test="position() = last() and position() &gt;= 5">
							<xsl:attribute name="style">border-right-width:1px;border-bottom-width:0px;</xsl:attribute>
						  </xsl:if>
						  <xsl:call-template name="PopulatePercent">
						...
						<td style="width:33mm;border-right-width:0px;" class="styDepTblCell">
						  <xsl:if test="position() = last() and position() &gt;= 5">
							<xsl:attribute name="style">border-right-width:0px;border-bottom-width:0px;</xsl:attribute>
						  </xsl:if>
						  <xsl:call-template name="PopulateAmount">
							<xsl:with-param name="TargetNode" select="AmountOfCompensation"/>
						  </xsl:call-template>
	  ...
	  <xsl:template name="Populateirs1125EemptyCells">
		...
		<tr>
		  <xsl:if test="($end = 'false')">
			<td class="styTableCell" style="width:50mm;text-align:left;font-weight:bold;font-size: 7pt;">
			  ...
			  <span style="font-weight: normal">
				<xsl:if test="($Print=$Separated) and (count($Form1125EData/Officer) &gt;5) and ($index = 1)">
		  ...
		  <xsl:if test="($end != 'false')">
			...
			<td style="width:21mm;border-right-width:1px;border-bottom-width:0px;" class="styTableCell">
			  <span style="width:1px;"/>
			</td>
			<td style="width:21mm;border-left-width:0px;border-bottom-width:0px;" class="styTableCell">
			  <span style="width:1px;"/>
			</td>
