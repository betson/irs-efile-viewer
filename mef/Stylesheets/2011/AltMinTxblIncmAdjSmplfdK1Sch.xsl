...
<xsl:template name="AltMinTxblIncmAdjSmplfdK1SchTemp">
    <table id="AltMinTxblIncmAdjSmplfdK1SchTbl" class="styDepTbl">
      <thead class="styTableThead">
        ...
        <tfoot></tfoot>
        <tbody>
        <xsl:for-each select="$AltMinTxblData/Schedule">  
          <tr>
            ...
            <xsl:if test="count(PYLongTermContractsFromK1)=0 or count (PYLongTermContractsFromK1)=1">
              ...
              <td class="styDepTblCell" scope="col"  style="text-align:right;font-size: 7pt; width: 40mm;">
                <xsl:call-template name="PopulateAmount">
                  <xsl:with-param name="TargetNode" select="PYLongTermContractsFromK1/YearlyInfo/PYAdjustmentFromK1" />
                </xsl:call-template>
              </td>
              <td class="styDepTblCell" scope="col"  style="text-align:right;font-size: 7pt; width: 40mm;">
                <xsl:call-template name="PopulateAmount">
                  <xsl:with-param name="TargetNode" select="PYLongTermContractsFromK1/YearlyInfo/PYNetAdjustmentToIncome" />
                </xsl:call-template>
              </td>
            </xsl:if>  
            <!-- If there is more than one set of data, show the first set with Row 1 background -->
            <xsl:if test="count(PYLongTermContractsFromK1)&gt;1">                        <!-- If there is more than one instance of data, fill the first cell with dark blue -->
          ...
          <xsl:for-each select="PYLongTermContractsFromK1">        
            <xsl:if test="position() &gt; 1">
              <tr>
                ...
                <td class="styDepTblCell" scope="col"  style="text-align:right;font-size: 7pt; width: 40mm">
                  <xsl:call-template name="PopulateAmount">
                    <xsl:with-param name="TargetNode" select="YearlyInfo/PYAdjustmentFromK1" />
                  </xsl:call-template>
                </td>
                <td class="styDepTblCell" scope="col"  style="text-align:right;font-size: 7pt; width: 40mm">
                  <xsl:call-template name="PopulateAmount">
                    <xsl:with-param name="TargetNode" select="YearlyInfo/PYNetAdjustmentToIncome" />
                  </xsl:call-template>
                </td>
              </tr>
            </xsl:if>
