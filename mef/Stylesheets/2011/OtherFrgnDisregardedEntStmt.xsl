...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:param name="DependencyData" select="$RtnDoc/OtherFrgnDisregardedEntStmt"/>
  <!-- Share Common Dependency - Other Foreign Disregarded Entities Statement -->
  <xsl:param name="depDocTitle">
    <xsl:call-template name="PopulateDisplayName">
      <xsl:with-param name="TargetNode" select="$DependencyData"/>
    </xsl:call-template>
  </xsl:param>
  <xsl:template name="OtherFrgnDisregardedEntStmtTemp2">
    <table id="OtherFrgnDisregardedEntStmtTbl2" class="styDepTblLandscape" style="font-size: 8pt">
  ...
  <xsl:template match="/">
    <html>
      ...
      <body class="styBodyClass">
        ...
        <xsl:call-template name="OtherFrgnDisregardedEntStmtTemp2"/>
      </body>
    </html>
  </xsl:template>
</xsl:stylesheet>
