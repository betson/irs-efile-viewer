  ...
  <xsl:template name="ShowPIS">
    <table id="PISTbl" class="styDepTblLandscape" style="text-align:center;">
      <thead class="styTableThead">
        ...
        <tfoot></tfoot>
       <tbody>    
        <xsl:for-each select="$CCSData/Sect199ClosingBooksElect">  
          <tr>
            ...
            <td class="styDepTblCell" style="width:61mm;text-align:left;">
              <xsl:call-template name="PopulateText">
                <xsl:with-param name="TargetNode" select="Section199BeginingText" />
              </xsl:call-template>
            </td>
            <!-- Column 2 -->
            ...
            <!--<br/>
            <xsl:call-template name="PopulateText">          
              <xsl:with-param name="TargetNode" select="$CCSData/Sect199ClosingBooksElect/CorporationName/BusinessNameLine2" />
            </xsl:call-template>-->
            </td>
            <!--Column 3 -->
           ...
           <!--Column 4 --> 
            <td class="styDepTblCell" style="width:60mm;text-align:left;">
              <xsl:call-template name="PopulateText">
                <xsl:with-param name="TargetNode" select="ResponsePeriodText" />
              </xsl:call-template>
            </td>
           <!-- Column 5 -->
            <td class="styDepTblCell" style="width:25mm;text-align:center;">
                <xsl:call-template name="PopulateMonthDayYear">
                  <xsl:with-param name="TargetNode" select="DateFrom" />
                </xsl:call-template>
            </td>
            <!-- Column 6 -->
            <td class="styDepTblCell" style="width:25mm;text-align:center;">
                <xsl:call-template name="PopulateMonthDayYear">
                  <xsl:with-param name="TargetNode" select="DateTo" />
                </xsl:call-template>
            </td>
          </tr>      
        </xsl:for-each>
