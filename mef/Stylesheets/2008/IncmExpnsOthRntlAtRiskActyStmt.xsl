...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:param name="IEOARSData" select="$RtnDoc/IncmExpnsOthRntlAtRiskActyStmt" />
  
  <!-- Template to display Itemized Income and Expenses from other Passive Rental Activities Statement-->
  
  <xsl:template name="ShowIEOARS">
    <!-- Itemized  Schedule Data -->
    <table id="IEOARSTbl" class="styDepTbl">
      ...
      <tr class="styDepTblHdr">         
        ...
        <tfoot></tfoot>
       <tbody>
      <xsl:for-each select="$IEOARSData/IncomeExpnsOtherRntlAtRiskActy">
        <tr>
  ...
  <xsl:param name="depDocTitle">
    <xsl:call-template name="PopulateDisplayName"><xsl:with-param name="TargetNode" select="$IEOARSData" /></xsl:call-template>  
  </xsl:param>
  <!-- Main template -->
  <xsl:template match="/">
    <html>
      ...
      <body class="styBodyClass" >
        ...
        <xsl:call-template name="PopulateDepCommonLeftover"><xsl:with-param name="TargetNode" select="$IEOARSData" /></xsl:call-template>        
                  
        <xsl:call-template name="ShowIEOARS" />          
        <br/>                      
      </body>
    </html>
  </xsl:template>
