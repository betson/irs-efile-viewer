...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:param name="DependencyData" select="$RtnDoc/OtherNonCurrentUSAssetsSch" />
  <!--HINTS - Display name is Other Non-Current US Assets Schedule- 1120F -->
  <xsl:param name="depDocTitle">
    <xsl:call-template name="PopulateDisplayName">
      <xsl:with-param name="TargetNode" select="$DependencyData" />
    </xsl:call-template>
  </xsl:param>
  <xsl:template name="OtherNonCurrentUSAssetsSchTemp">
  <table id="GeneralDependencyTbl" class="styDepTblLandscape">
    <tr class="styDepTblHdr">
      ...
      <xsl:choose>
        <xsl:when test="$DependencyData/OtherNonCurrentUSAssets/CorporationEIN">
        ...
        <xsl:otherwise>
          ...
          <th class="styDepTblCell" style="width:86mm;text-align:center;" scope="col">Other Non-Current US Assets Description</th>
    ...
    <tfoot/>
    <tbody>
    <xsl:for-each select="$DependencyData/OtherNonCurrentUSAssets">
      <tr>
         ...
         <!--Column 1 - Corporation Name -->
          <td class="styDepTblCell" style="width:85mm;text-align:left;vertical-align:middle;font-size:7pt">
            ...
            <xsl:attribute name="rowspan">
              <xsl:value-of select="count(OtherNonCurrentUSAssetsLnItem)"></xsl:value-of>
            </xsl:attribute>
            <!--Allow row to expand by column -->
          ...
          <xsl:if test="count(OtherNonCurrentUSAssetsLnItem)=0 or count (OtherNonCurrentUSAssetsLnItem)=1">
            <!--Column 3 - Description -->
            <td class="styDepTblCell" style="width:86mm;text-align:left;font-size:7pt">
              <xsl:call-template name="PopulateText">
                <xsl:with-param name="TargetNode" select="OtherNonCurrentUSAssetsLnItem/Description" />
              </xsl:call-template>
              <span class="styTablecellpad"/>
            </td>
            <!--Column 3 - Description -->
            ...
            <td class="styDepTblCell" style="width:30mm;text-align:right;font-size:7pt">
              <xsl:call-template name="PopulateAmount">
                 <xsl:with-param name="TargetNode" select="OtherNonCurrentUSAssetsLnItem/BeginingAmount" />
              </xsl:call-template>
              <span class="styTablecellpad"/>
            </td>
            <!--Column 4 - Beginning Amount -->
            ...
            <td class="styDepTblCell" style="width:30mm;text-align:right;font-size:7pt">
              <xsl:call-template name="PopulateAmount">
                <xsl:with-param name="TargetNode" select="OtherNonCurrentUSAssetsLnItem/EndingAmount" />
              </xsl:call-template>
              <span class="styTablecellpad"/>
            </td>
            <!--Column 5 - Ending Amount -->
          ...
          <xsl:if test="count(OtherNonCurrentUSAssetsLnItem)&gt;1">
      ...
      <xsl:for-each select="OtherNonCurrentUSAssetsLnItem">
  ...
  <xsl:template match="/">
    <html>
      ...
      <body class="styBodyClass">
        ...
        <xsl:call-template name="OtherNonCurrentUSAssetsSchTemp" />
      </body>
    </html>
  </xsl:template>
</xsl:stylesheet>
