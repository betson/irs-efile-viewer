...
<!--          <th class="styTblCell" scope="col">Item No.</th>       -->
      ...
      <xsl:for-each select="$DependencyData/REMIC">
        <tr>
...
<!--          <td class="styDepTblCell" style="text-align:center;" >
          ...
          <td class="styDepTblCell" style="text-align:right;" >
            <xsl:call-template name="PopulateAmount">          
                 <xsl:with-param name="TargetNode" select="ShareOfTaxableIncome" />
            </xsl:call-template>
          </td>      
          <td class="styDepTblCell" style="text-align:right;" >
            <xsl:call-template name="PopulateAmount">          
                 <xsl:with-param name="TargetNode" select="ExcessInclusion" />
            </xsl:call-template>
          </td>
          <td class="styDepTblCell" style="text-align:right;" >  
            <xsl:call-template name="PopulateAmount">          
                 <xsl:with-param name="TargetNode" select="Section212Expenses" />
            </xsl:call-template>
          </td>                
        </tr>
      </xsl:for-each>
