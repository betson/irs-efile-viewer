...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:param name="DependencyData" select="$RtnDoc/ContributionsSchedule" />
  ...
  <xsl:template name="ShowDependencyData">
    ...
    <table id="DependencyDataTbl" class="styDepTbl">
      ...
      <tr class="styDepTblHdr">         
       ...
       <tbody>
      <xsl:for-each select="$DependencyData/Contribution">
        <tr>
          ...
          <xsl:choose>
           ...
           <td class="styDepTblCell" style="text-align:left" width="320mm">
            <xsl:call-template name="PopulateText">
              <xsl:with-param name="TargetNode" select="Description"/>
            </xsl:call-template>
          </td>        
          <td class="styDepTblCell" style="text-align:right" width="105mm">
            <xsl:call-template name="PopulateAmount">
              <xsl:with-param name="TargetNode" select="Amount"/>
            </xsl:call-template>
          </td>     
          <td class="styDepTblCell" style="text-align:left" width="217mm">
            <xsl:call-template name="PopulateText">
