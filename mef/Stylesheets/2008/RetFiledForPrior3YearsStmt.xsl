      ...
      <tbody>
        <xsl:for-each select="$DependencyData/RetFiledForPriorYear">
          <tr>
            ...
            <td class="styDepTblCell" style="text-align:left">
              <xsl:call-template name="PopulateText">
                <xsl:with-param name="TargetNode" select="CorpNameFilingConsolidatedRet/BusinessNameLine1"/>
              </xsl:call-template>
              <xsl:if test="CorpNameFilingConsolidatedRet/BusinessNameLine2 !=''">
                <br/>
                <xsl:call-template name="PopulateText">
                  <xsl:with-param name="TargetNode" select="CorpNameFilingConsolidatedRet/BusinessNameLine2"/>
                </xsl:call-template>
            </xsl:if>
            </td>
            ...
            <td class="styDepTblCell" style="text-align:right;">
              <xsl:call-template name="PopulateAmount">
                <xsl:with-param name="TargetNode" select="USIncomeTaxPaid" />
              </xsl:call-template>
            </td>
          </tr>
        </xsl:for-each>
