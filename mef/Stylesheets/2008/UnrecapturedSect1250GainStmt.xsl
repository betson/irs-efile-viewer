  ...
  <xsl:template name="ShowDependencyData">
    ...
    <table class="styDepTbl">
      <thead class="styTableThead">
        ...
        <tfoot></tfoot>
       <tbody>
        <xsl:for-each select="$DependencyData/UnrecapturedSect1250Gain">
          <tr>
            ...
            <td class="styDepTblCell"  style="width:62mm;text-align:right">
              <xsl:call-template name="PopulateAmount">
                <xsl:with-param name="TargetNode" select="IntrestInPartnershipAmount"/>
              </xsl:call-template>
            </td>    
            <td class="styDepTblCell" style="width:63mm;text-align:right">
              <xsl:call-template name="PopulateAmount">
                <xsl:with-param name="TargetNode" select="EstateTrustREITAmount"/>
              </xsl:call-template>
            </td>            
          </tr>
        </xsl:for-each>
