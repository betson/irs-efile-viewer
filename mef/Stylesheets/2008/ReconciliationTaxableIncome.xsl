...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:param name="CPSData" select="$RtnDoc/ReconciliationTaxableIncome" />
  <!-- The dependency name is Reconciliation of Schedule M-3 Taxable Income with Tax Return Taxable Income for Mixed Groups -->    
        ...
        <div class="styTopSectionLine">
          ...
          <div style="float:left;clear:none;">
            <xsl:call-template name="PopulateText">
              <xsl:with-param name="TargetNode" select="$CPSData/Explanation"/>
            </xsl:call-template>            
          </div>
        </div>
        <br />      
