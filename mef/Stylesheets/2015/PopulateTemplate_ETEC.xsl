...
<xsl:template name="PopulateUSAddressTemplate">
  ...
  <xsl:choose>
			<xsl:when test="$Type = '2290' or $Type = '720' or $Type = '8849' or $Type = '941' or $Type = '941PR' or $Type = '941SS'">
...
<xsl:template name="PopulateReturnHeaderFiler">
  ...
  <xsl:when test="$Location='SUB'"> <!-- Indicating that data needs to be pulled from Subsidiary return header-->
    ...
    <xsl:if test="$SubRtnHdrData/SubsidiaryCorp">
      <xsl:choose>
        <xsl:when test="$TargetNode='BusinessNameLine1' and not($SubRtnHdrData/SubsidiaryCorpGrp/Name/BusinessNameLine1)">
          <xsl:call-template name="PopulateText">
            <xsl:with-param name="TargetNode" select="$SubRtnHdrData/SubsidiaryCorpGrp/Name" />
          </xsl:call-template>
        </xsl:when>
    ...
    <xsl:otherwise> <!-- Indicating that the data needs to be pulled from Filer return header-->
        <xsl:choose>
            ...
            <xsl:when test="$TargetNode='BusinessNameLine1' and not($RtnHdrData/Filer/BusinessName/BusinessNameLine1)">
              <xsl:choose>
              <xsl:when test="$Type != '1041' and $Type != '2290' and $Type != '720' and $Type != '8849' and $Type != '941' and $Type != '941PR' and $Type != '941SS'">
              <xsl:choose>
            ...
            <xsl:when test="$TargetNode='BusinessNameLine2' and not($RtnHdrData/Filer/BusinessName/BusinessNameLine2)">
            <xsl:choose>
              <xsl:when test="$Type != '1041' and $Type != '2290' and $Type != '720' and $Type != '8849' and $Type != '941' and $Type != '941PR' and $Type != '941SS'">
				<xsl:call-template name="PopulateText">
...
-->
  <xsl:template name="SetInitialState">
    <xsl:if test="($Print='inline' or $Print='separated')">
      <xsl:attribute name="style">overflow:visible;height:auto;</xsl:attribute>
    </xsl:if>
  </xsl:template>
