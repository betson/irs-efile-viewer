...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:param name="DependencyData" select="$RtnDoc/InvestmentsOtherSchedule2"/>
  ...
  <xsl:template name="InvestmentsOtherSchedule2Temp">
    <table id="InvestmentsOtherSchedule2Tbl" class="styDepTbl">
      <thead class="styTableThead">
        <tr class="styDepTblHdr">
          <th class="styDepTblCell" scope="col">Category/ Item</th>
          <th class="styDepTblCell" scope="col">Listed at Cost or FMV</th>
          <th class="styDepTblCell" scope="col">Book Value</th>
          <th class="styDepTblCell" scope="col">End of Year Fair Market Value</th>
        </tr>
      </thead>
      <xsl:for-each select="$DependencyData/InvestmentsOtherGrp">
        <tr>
          ...
          <td class="styDepTblCell" style="text-align:left;">
            <xsl:call-template name="PopulateText">
              <xsl:with-param name="TargetNode" select="CategoryOrItemTxt"/>
            </xsl:call-template>
          </td>          
          <td class="styDepTblCell" style="text-align:left;width:30mm">
            <xsl:call-template name="PopulateText">
              <xsl:with-param name="TargetNode" select="ListedAtCostOrFMVCd"/>
            </xsl:call-template>
          </td>
  ...
  <xsl:template match="/">
    <xsl:text disable-output-escaping="yes">&lt;!DOCTYPE html&gt;</xsl:text>
      ...
      <!-- NameDisplay - IRS990PF - Investments - other schedule -->
      <body class="styBodyClass">      
        ...
        <xsl:call-template name="InvestmentsOtherSchedule2Temp"/>
      </body>
    </html>
  </xsl:template>
</xsl:stylesheet>