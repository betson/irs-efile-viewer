...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:param name="FCSData" select="$RtnDoc/Form1120ComputationSchedule"/>

  <xsl:template name="ShowFCS">    
    <table id="FCSTbl" class="styDepTbl">
    ...
    <tfoot/>
      ...
      <tbody>    
      <xsl:for-each select="$FCSData/ComputationInformationGrp">  
          <tr>
  ...
  <xsl:param name="depDocTitle">
    <xsl:call-template name="PopulateDisplayName"><xsl:with-param name="TargetNode" select="$FCSData"/></xsl:call-template>  
  </xsl:param>
  <!-- Main template -->
  <xsl:template match="/">
    <xsl:text disable-output-escaping="yes">&lt;!DOCTYPE html&gt;</xsl:text>
      ...
      <body class="styBodyClass">
        ...
        <xsl:call-template name="PopulateDepCommonLeftover"><xsl:with-param name="TargetNode" select="$FCSData"/></xsl:call-template>      
                  
        <xsl:call-template name="ShowFCS"/>          
        <br/>                      
      </body>
    </html>
  </xsl:template>
