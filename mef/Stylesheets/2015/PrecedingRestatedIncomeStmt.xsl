    ...
    <xsl:text disable-output-escaping="yes">&lt;!DOCTYPE html&gt;</xsl:text>
      ...
      <body class="styBodyClass">    
        ...
        <table class="styDepTbl" style="margin-top:4mm;">
          ...
          <tfoot/>
          <tbody>    
            <xsl:for-each select="$DependencyData/PrecedingRestateIncStmtGrp">
            <tr>
                ...
                <td class="styDepTblCell" style="width:60mm;text-align:left;">
                      <xsl:call-template name="PopulateText">
                        <xsl:with-param name="TargetNode" select="RestatedAnnualIncomeStmtPrdTxt"/>
                </xsl:call-template>
                </td>
                <td class="styDepTblCell" style="width:33mm;text-align:right;">
                <xsl:call-template name="PopulateAmount">
                          <xsl:with-param name="TargetNode" select="OriginalIncomeAmt"/>
                </xsl:call-template>  
                </td>
                <td class="styDepTblCell" style="width:33mm;text-align:right;">
                      <xsl:call-template name="PopulateAmount">
                        <xsl:with-param name="TargetNode" select="RestatedIncomeAmt"/>
                      </xsl:call-template>
          </td>        
        <td class="styDepTblCell" style="width:60mm;text-align:left;">
                      <xsl:call-template name="PopulateText">
                        <xsl:with-param name="TargetNode" select="RestatementDesc"/>
                      </xsl:call-template>
              </td>                    
            </tr>
          </xsl:for-each>
