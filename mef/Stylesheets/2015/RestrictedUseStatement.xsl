...
<!DOCTYPE xsl:stylesheet [<!ENTITY nbsp "&#160;">]><xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:param name="FormData" select="$RtnDoc/RestrictedUseStatement"/>
 <!-- Common  - Bad Debts - Current Year's Provision Schedule -   -->
        ...
        <tr>
          <td class="styDepTblCell" style="border:none;text-align:left">            
             <xsl:call-template name="PopulateText">
                <xsl:with-param name="TargetNode" select="$FormData/RestrictedUseAgreementDetail/ExplanationTxt"/>
              </xsl:call-template>            
          </td>                      
        </tr>
    </table>
    ...
    <xsl:text disable-output-escaping="yes">&lt;!DOCTYPE html&gt;</xsl:text>
      ...
      <body class="styBodyClass">    
		...
		<div class="styTopSectionLine"> 
          <div class="styTopSectionLineLbl">Agreement: </div>
	   </div>
