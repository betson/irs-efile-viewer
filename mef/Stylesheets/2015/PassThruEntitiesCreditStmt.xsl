...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:param name="PTECSData" select="$RtnDoc/PassThruEntitiesCreditStmt" />
    <xsl:param name="depDocTitle">
    <xsl:call-template name="PopulateDisplayName">
      <xsl:with-param name="TargetNode" select="$PTECSData" />
    </xsl:call-template>  
  </xsl:param>
  ...
  <xsl:template match="/">
    <html>
      ...
      <body class="styBodyClass" >
        ...
        <xsl:call-template name="ShowPTECS" />          
        <br/>                      
      </body>
    </html>
  </xsl:template>
  ...
  <xsl:template name="ShowPTECS">
    <table id="PTECSTbl" class="styDepTbl">
      ...
      <tfoot></tfoot>
      <tbody>    
        <xsl:for-each select="$PTECSData/PassThroughEntitiesCredit">  
          <tr>
