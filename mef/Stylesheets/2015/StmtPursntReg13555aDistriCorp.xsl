        ...
        <div class="styTopSectionLine" style="width:256mm;">
          ...
          <div style="float:left;clear:none;">
            <xsl:call-template name="PopulateText">
              <xsl:with-param name="TargetNode" select="$DependencyData/ControlledCorporationName/BusinessNameLine1Txt"/>
            </xsl:call-template>
            <br/>
            <xsl:call-template name="PopulateText">
              <xsl:with-param name="TargetNode" select="$DependencyData/ControlledCorporationName/BusinessNameLine2Txt"/>
            </xsl:call-template>
          </div>
        </div>
              ...
              <td class="styDepTblCell" style="width:54mm;text-align:left;">
                <xsl:choose>
                  <xsl:when test="SignificantDistributeeBusName">
                    <xsl:call-template name="PopulateText">
                      <xsl:with-param name="TargetNode" select="SignificantDistributeeBusName/BusinessNameLine1Txt"/>
                    </xsl:call-template>
                    <xsl:if test="normalize-space(SignificantDistributeeBusName/BusinessNameLine2Txt)!=''">
                      <br/>
                      <xsl:call-template name="PopulateText">
                        <xsl:with-param name="TargetNode" select="SignificantDistributeeBusName/BusinessNameLine2Txt"/>
                      </xsl:call-template>
                    </xsl:if>
                  </xsl:when>
                  <xsl:otherwise>
