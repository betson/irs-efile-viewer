...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:param name="DependencyData" select="$RtnDoc/USNetEquityEndOfCurrentYearSch"/>
  <!-- - Form 1120F - Displays as - US Net Equity End Of Current Year Schedule - -->
  <xsl:param name="depDocTitle">
    <xsl:call-template name="PopulateDisplayName">
      <xsl:with-param name="TargetNode" select="$DependencyData"/>
    </xsl:call-template>
  </xsl:param>
  <xsl:template name="USNetEquityEndOfCurrentYearSchTemp">
  ...
  <xsl:template match="/">
    <xsl:text disable-output-escaping="yes">&lt;!DOCTYPE html&gt;</xsl:text>
      ...
      <body class="styBodyClass">
        ...
        <xsl:call-template name="USNetEquityEndOfCurrentYearSchTemp"/>
      </body>
    </html>
  </xsl:template>
</xsl:stylesheet>