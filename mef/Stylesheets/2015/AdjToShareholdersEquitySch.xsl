...
<xsl:param name="DependencyData" select="$RtnDoc/AdjToShareholdersEquitySch"/>
<xsl:param name="depDocTitle">
  <xsl:call-template name="PopulateDisplayName"><xsl:with-param name="TargetNode" select="$DependencyData"/></xsl:call-template>  
</xsl:param>
<!-- - Common Form displays as - Adjustment To Reconcile Income Schedule - -->
<xsl:template name="AdjToShareholdersEquitySchTemp">
    <table id="AdjustmentsToShareholdersEquityScheduleTbl" class="styDepTbl">
      <thead class="styTableThead">
        ...
        <tfoot/>
        <tbody>
        <xsl:for-each select="$DependencyData/AdjToShareholdersEquity">  
          <tr>
            ...
            <td class="styDepTblCell" scope="col" style="text-align:left;font-size: 7pt;width:35mm;">
              <xsl:attribute name="rowspan">
                <xsl:value-of select="count(AdjToEquityLineItem)"/>
              </xsl:attribute>
            ...
            <xsl:for-each select="AdjToEquityLineItem">
  ...
  <xsl:template match="/">
    <xsl:text disable-output-escaping="yes">&lt;!DOCTYPE html&gt;</xsl:text>
      ...
      <body class="styBodyClass">
        ...
        <xsl:call-template name="AdjToShareholdersEquitySchTemp"/>
      </body>
    </html>
  </xsl:template>
</xsl:stylesheet>