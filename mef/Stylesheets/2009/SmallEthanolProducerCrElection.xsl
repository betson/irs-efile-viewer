...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template name="ShowDependencyData">  
    <table id="DependencyDataTbl" class="styDepTbl" style="font-size: 8pt">
      ...
      <tfoot></tfoot>        
      <tbody>
        <xsl:for-each select="$DependencyData/SmallEthanolProducerCrElect">
          <tr>
              ...
              <xsl:choose>
    	       ...
    	       <td class="styDepTblCell" style="text-align:right; width: 62mm">      		
    	          <xsl:call-template name="PopulateAmount">
    	            <xsl:with-param name="TargetNode" select="TotGallonsQlfyEthanolFuelPrdcd"/>
    	          </xsl:call-template> 
    		    </td>       
    	       <td class="styDepTblCell" style="text-align:right; width: 62mm">      		
    	          <xsl:call-template name="PopulateAmount">
    	            <xsl:with-param name="TargetNode" select="AmountAllocatedToCooperative"/>
    	          </xsl:call-template> 
    		    </td>  
  	         <td class="styDepTblCell" style="text-align:right; width: 62mm">      		
  	          <xsl:call-template name="PopulateAmount">
  	            <xsl:with-param name="TargetNode" select="QualifiedEthanolFuelProduction"/>
  	          </xsl:call-template> 
  		      </td>  			  
          </tr>
        </xsl:for-each>
