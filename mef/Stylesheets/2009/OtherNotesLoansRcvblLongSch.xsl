...
<!-- end change -->
  ...
  <xsl:template name="ShowDS">
    ...
    <table id="DSTbl" class="styDepTblLandscape" style="font-size: 6pt">  
      ...
      <tfoot></tfoot>    
      <tbody>
        <xsl:for-each select="$Form990DepsData/OtherNotesLoansRcvblLong">
        <tr>
          ...
          <td class="styDepTblCell" style="text-align:left">
            <xsl:if test="BorrowersName/Individual != ''">          
              <xsl:call-template name="PopulateText"><xsl:with-param name="TargetNode" select="BorrowersName/Individual" /></xsl:call-template> 
            </xsl:if>
            <xsl:if test="BorrowersName/Business/BusinessNameLine1 != ''">
              <xsl:call-template name="PopulateText"><xsl:with-param name="TargetNode" select="BorrowersName/Business/BusinessNameLine1" />
              </xsl:call-template> 
            </xsl:if>  
            <xsl:if test="BorrowersName/Business/BusinessNameLine2 != ''"><br/>          
          ...
          <td class="styDepTblCell" style="text-align:right">
            <xsl:call-template name="PopulateAmount"><xsl:with-param name="TargetNode" select="OriginalAmountOfLoan" /></xsl:call-template>
          </td>
          <td class="styDepTblCell" style="text-align:right" >
            <xsl:call-template name="PopulateAmount"><xsl:with-param name="TargetNode" select="BalanceDue" /></xsl:call-template>
          </td>
          ...
          <td class="styDepTblCell" style="text-align:right">
            <xsl:call-template name="PopulateAmount"><xsl:with-param name="TargetNode" select="ConsiderationFmv" /></xsl:call-template>           
          </td>
        </tr>
        </xsl:for-each>
      </tbody>
