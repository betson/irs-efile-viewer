...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
         ...
         <div class="styBB" style="width:187mm;">
           ...
           <div class="styNameBox" style="width:30mm;font-size:7pt;border-right:0px solid black;">
	              <span style="padding-left:1mm;">Telephone number</span><br/><br/>
	                    ...
	                    <xsl:call-template name="PopulatePhoneNumber">
		                  <xsl:with-param name="TargetNode" select="$Form8308Data/TelephoneNumber"/>
		              </xsl:call-template>    
		       </span>
	        </div>
            ...
            <div class="styNameBox" style="width:155mm;height:7mm;font-size:7pt;">
              Name<br/>
                   <xsl:call-template name="PopulateText">
                    <xsl:with-param name="TargetNode" select="$Form8308Data/NameOfTransferor/BusinessNameLine1"/>
                  </xsl:call-template>
                  <xsl:if test="$Form8308Data/NameOfTransferor/BusinessNameLine2">
                    <br/>
                    <xsl:call-template name="PopulateText">
                      <xsl:with-param name="TargetNode" select="$Form8308Data/NameOfTransferor/BusinessNameLine2"/>
                    </xsl:call-template>
                  </xsl:if>
          </div>

          <div style="height: 7mm;padding-left:2mm;font-size:7pt; font-weight: normal;" class="styEINBox">         
            <span class="styBoldText">Identifying number</span><br/><br/>   
                  <xsl:choose>
                   <xsl:when test="$Form8308Data/TransferorEINIDNumber">
                     <xsl:call-template name="PopulateEIN">
		                  <xsl:with-param name="TargetNode" select="$Form8308Data/TransferorEINIDNumber"/>
		                </xsl:call-template>
                   </xsl:when>                  
                   <xsl:otherwise>
                     <xsl:call-template name="PopulateSSN">
		                  <xsl:with-param name="TargetNode" select="$Form8308Data/TransferorSSNIDNumber"/>
	                </xsl:call-template>
                  </xsl:otherwise>
                </xsl:choose>
          </div>
          ...
          <div class="styBB" style="width:187mm;">
            <div class="styNumberBox" style="height:8mm;font-size:7pt;">
              ...
              <br/>
             	 <xsl:call-template name="PopulateText">
         	       <xsl:with-param name="TargetNode" select="$Form8308Data/TransferorUSAddress/AddressLine1"/>
        	      </xsl:call-template>
             	 <xsl:if test="$Form8308Data/TransferorUSAddress/AddressLine2">
                <br/>
                <xsl:call-template name="PopulateText">
                  <xsl:with-param name="TargetNode" select="$Form8308Data/TransferorUSAddress/AddressLine2"/>
                </xsl:call-template>
               </xsl:if>
            </div>
          </div>
  ...
  <div class="styBB" style="width:187mm;float:none;clear:both;">
	...
	<div class="styLNDesc" style="width:186mm;height:4mm;">
	  		...
	  		<i>The information on this form has been supplied to the Internal Revenue Service. The transferor in a section 751(a)
	  		exchange is required to treat a portion of the gain realized from the exchange as ordinary income. For more details, 
	  		see Pub. 541, Partnerships.</i>
  	</div>          
	 
	<div class="styLNDesc" style="width:186mm;height:4mm;">
	 		<b>Statement by Transferor:</b>
	 		<i>The transferor in a section 751(a) exchange is required under Regulations section 1.751-1(a)(3)
	 		 to attach a statement relating to the sale or exchange to his or her return. See Instructions to Transferors for more details.
 	 		</i> 
	</div>
   </div>      
   ...
   <div class="styBB" style="width:187mm;float:none;clear:both;">
     ...
     <span class="styNormalText" style=""> (Beneficial owner of the partnership interest immediately after the transfer of that interest)</span></span>
   </div>
   ...
   <div class="styBB" style="width:187mm;">
      ...
      Name<br/>
      <xsl:call-template name="PopulateText">
         <xsl:with-param name="TargetNode" select="$Form8308Data/NameOfTransferee/BusinessNameLine1"/>
      </xsl:call-template>
      <xsl:if test="$Form8308Data/NameOfTransferee/BusinessNameLine2">
      <br/>
      <xsl:call-template name="PopulateText">
         <xsl:with-param name="TargetNode" select="$Form8308Data/NameOfTransferee/BusinessNameLine2"/>
      </xsl:call-template>
      </xsl:if>
    </div>
    ...
    <div style="height: 7mm;padding-left:2mm;font-size:7pt; font-weight: normal;" class="styEINBox">         
       ...
       <xsl:choose>
       <xsl:when test="$Form8308Data/TransfereeEINIDNumber">
       <xsl:call-template name="PopulateEIN">
		  <xsl:with-param name="TargetNode" select="$Form8308Data/TransfereeEINIDNumber"/>
		  </xsl:call-template>
       </xsl:when>                  
       <xsl:otherwise>

       <xsl:call-template name="PopulateSSN">
		  <xsl:with-param name="TargetNode" select="$Form8308Data/TransfereeSSNIDNumber"/>
		  </xsl:call-template>
       </xsl:otherwise>
       </xsl:choose>
    ...
    <div class="styBB" style="width:187mm;">
      <div class="styNumberBox" style="height:8mm;font-size:7pt;">
        ...
        <xsl:call-template name="PopulateText">
        <xsl:with-param name="TargetNode" select="$Form8308Data/TransfereeUSAddress/AddressLine1"/>
        </xsl:call-template>
        <br/>
        <xsl:call-template name="PopulateText">
        <xsl:with-param name="TargetNode" select="$Form8308Data/TransfereeUSAddress/AddressLine2"/>
        </xsl:call-template>
        </xsl:if>
        <xsl:if test="$Form8308Data/TransfereeForeignAddress">
        <xsl:call-template name="PopulateText">
        <xsl:with-param name="TargetNode" select="$Form8308Data/TransfereeForeignAddress/AddressLine1"/>
        </xsl:call-template>
        <br/>
        <xsl:call-template name="PopulateText">
        <xsl:with-param name="TargetNode" select="$Form8308Data/TransfereeForeignAddress/AddressLine2"/>
        </xsl:call-template>
        </xsl:if>
    ...
    <div class="styBB" style="width:187mm;float:none;clear:both;height:6mm;">
       ...
       <xsl:call-template name="PopulateMonthDayYear">
		  <xsl:with-param name="TargetNode" select="$Form8308Data/DateOfSaleOrExchange"/>
		  </xsl:call-template>
		  <br/>
			</span>
    </div>
