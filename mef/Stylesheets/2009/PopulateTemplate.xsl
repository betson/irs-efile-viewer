    ...
    <xsl:otherwise> <!-- Indicating that the data needs to be pulled from Filer return header -->
        ...
        <xsl:when test="$TargetNode = 'Phone' ">    
          <xsl:call-template name="PopulatePhoneNumber">
            ...
            <xsl:when test="$TargetNode = 'SSN' or $TargetNode = 'PrimarySSN' or $TargetNode = 'SpouseSSN' ">                
              <xsl:call-template name="PopulateSSN">
  ...
  <xsl:param name="BackupName" />    
    <xsl:choose>
      ...
      <xsl:when test="$TargetNode ='DateSigned'">
        <xsl:call-template name="PopulateMonthDayYear">
          <xsl:with-param name="TargetNode" select="$RtnHdrData/PaidPreparerInformation/DateSigned" />
        </xsl:call-template>
      </xsl:when>
      <xsl:when test="$TargetNode='BusinessNameLine1' and not(($RtnHdrData/PaidPreparerInformation/PreparerBusinessName/BusinessNameLine1) 
			  or ($RtnHdrData/PaidPreparerInformation/PreparerFirmName/BusinessNameLine1))">
        <xsl:call-template name="PopulateText">
          <xsl:with-param name="TargetNode" select="$RtnHdrData/PaidPreparerInformation/PreparerPersonName" />
        </xsl:call-template>
      </xsl:when>
      <xsl:when test="$TargetNode ='SSN'">
        <xsl:if test="$RtnHdrData/PaidPreparerInformation/SSN">
          <xsl:call-template name="PopulateSSN">
            <xsl:with-param name="TargetNode" select="$RtnHdrData/PaidPreparerInformation/SSN"/>
          </xsl:call-template>
        </xsl:if>
      </xsl:when>
      <xsl:when test="$TargetNode ='PTIN'">
        <xsl:if test="$RtnHdrData/PaidPreparerInformation/PTIN">
          <xsl:call-template name="PopulateText">
            <xsl:with-param name="TargetNode" select="$RtnHdrData/PaidPreparerInformation/PTIN"/>
          </xsl:call-template>
        </xsl:if>
      </xsl:when>
       <xsl:when test="$TargetNode ='PreparerFirmIDNumber'">
        <xsl:if test="$RtnHdrData/PaidPreparerInformation/PreparerFirmIDNumber">
          <xsl:call-template name="PopulateEIN">
            <xsl:with-param name="TargetNode" select="$RtnHdrData/PaidPreparerInformation/PreparerFirmIDNumber"/>
          </xsl:call-template>
        </xsl:if>
      </xsl:when>           
      <xsl:when test="$TargetNode ='Phone'">
        <xsl:if test="$RtnHdrData/PaidPreparerInformation/Phone">
          <xsl:call-template name="PopulatePhoneNumber">
            <xsl:with-param name="TargetNode" select="$RtnHdrData/PaidPreparerInformation/Phone"/>
          </xsl:call-template>
        </xsl:if>
      </xsl:when>
      <xsl:when test="$TargetNode ='ForeignPhone'">
        <xsl:if test="$RtnHdrData/PaidPreparerInformation/ForeignPhone">
          <xsl:call-template name="PopulateText">
            <xsl:with-param name="TargetNode" select="$RtnHdrData/PaidPreparerInformation/ForeignPhone"/>
          </xsl:call-template>
        </xsl:if>
      </xsl:when>
      <xsl:when test="$TargetNode='CityStateInfo' ">
          <xsl:call-template name="PopulateCityStateInfo">
              <xsl:with-param name="TargetNode" select="$RtnHdrData/PaidPreparerInformation/PreparerUSAddress" />
              <xsl:with-param name="ForeignAddressTargetNode" select="$RtnHdrData/PaidPreparerInformation/PreparerForeignAddress" />
          </xsl:call-template>
      </xsl:when>
      <xsl:otherwise>
      <!-- This clause handles all other elements such as 'Name','EmailAddress','DatePrepared'-->
        <xsl:call-template name="PopulateText">
          <xsl:with-param name="TargetNode" select="$RtnHdrData/PaidPreparerInformation//*[name(.)=$TargetNode]" />
        </xsl:call-template>
      </xsl:otherwise>      
    </xsl:choose>    
</xsl:template>
...
Called By:      Stylesheets
Calls:          none
Log:			the original version of SetFormLinkInLine
***************************************************************************************************************************************************************
-->
