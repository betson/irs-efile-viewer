    ...
    <table id="AdvncPymtGdsLongTermCntrctSchTbl" class="styDepTbl" style="font-size:7pt;">
      <thead class="styTableThead">
        <tr class="styDepTblHdr">
           ...
           <th class="styDepTblCell"  scope="col">Total amount of advance payments received in prior TY not included in gross income before the current TY</th>
      ...
      <xsl:for-each select="$DependencyData/AdvancePaymentContractSch">
        ...
        <td class="styDepTblCell" style="text-align:right;width:30mm;" >
          <xsl:call-template name="PopulateAmount">
            <xsl:with-param name="TargetNode" select="TotalAdvancePaymentsReceivedTY" />
          </xsl:call-template>
        </td>
        <td class="styDepTblCell" style="text-align:right;width:30mm;" >
          <xsl:call-template name="PopulateAmount">
            <xsl:with-param name="TargetNode" select="PymtRcvdPriorTYNotIncldGroIncm" />
          </xsl:call-template>
        </td>
        <td class="styDepTblCell" style="text-align:right;width:30mm;" >
          <xsl:call-template name="PopulateAmount">
            <xsl:with-param name="TargetNode" select="PymtRcvdPriorTYIncldGrossIncm" />
          </xsl:call-template>
        </td>
        </tr>
      </xsl:for-each>
