...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template name="RowWriterCore">
      <tr>
        ...
        <td class="IRS1118ScheduleI_GenericCell">  &nbsp;</td>
        <td class="IRS1118ScheduleI_GenericCell">  &nbsp;</td>
        <td class="IRS1118ScheduleI_GenericCell" style="width:45mm;border-right-width:0px;padding-right:2mm">&nbsp;</td>
      </tr>
  </xsl:template>
  <xsl:template name="AddRowsForSchAPartI">
    ...
    <tr>
      ...
      <xsl:if test="$TargetNode = 'A'">
			...
			<xsl:choose>
				<xsl:when test="($FormData/ForeignOilGasExtIncome/ForeignCountryOrUSPossessionCD)">
					<xsl:call-template name="PopulateAdditionalDataTableMessage">
						<xsl:with-param name="TargetNode" select="$FormData/ForeignOilGasExtIncome/ForeignCountryOrUSPossessionCD"/>
					</xsl:call-template>
				</xsl:when>
			   <xsl:otherwise>
				<xsl:call-template name="PopulateAdditionalDataTableMessage"> 		   
					<xsl:with-param name="TargetNode" select="$FormData/ForeignOilGasExtIncome/ForeignCountryOrUSPoss863B"/>
				</xsl:call-template>				
			   </xsl:otherwise>
			</xsl:choose>
        	</xsl:if>
...
<input type="checkbox" class="styCkbox">
	<xsl:call-template name="PopulateCheckbox">
		<xsl:with-param name="TargetNode" select="$FormData/PassiveCategoryIncome"/>
...
<input type="checkbox" class="styCkbox">
	<xsl:call-template name="PopulateCheckbox">
		<xsl:with-param name="TargetNode" select="$FormData/GeneralCategoryIncome"/>
...
<div style="height:12mm;width:108mm;float:left;">
	<input type="checkbox" class="styCkbox">
		<xsl:call-template name="PopulateCheckbox">
			<xsl:with-param name="TargetNode" select="$FormData/Section901jIncome"/>
	...
	<span style="border-color:black; border-style:solid; border-right-width: 0px; border-left-width: 0px; border-top-width:0px; border-bottom-width: 1px; width: 25mm">
		<xsl:call-template name="PopulateText">
			<xsl:with-param name="TargetNode" select="$FormData/Section901jIncome/@nameOfSanctionedCountry"/>
		</xsl:call-template>
	</span>
	<br/>
	<br/>
	<input type="checkbox" class="styCkbox">
		<xsl:call-template name="PopulateCheckbox">
			<xsl:with-param name="TargetNode" select="$FormData/IncomeResourcedByTreaty"/>
	...
	<label>
		...
		<span style="border-color:black; border-style:solid; border-right-width: 0px; border-left-width: 0px; border-top-width:0px; border-bottom-width: 1px; width: 31mm">
			<xsl:call-template name="PopulateText">
				<xsl:with-param name="TargetNode" select="$FormData/IncomeResourcedByTreaty/@nameOfCountry"/>
			</xsl:call-template>
		</span>
	</div>
  </div>
  ...
  <!-- end -->
     <table cellspacing="0" style="font-size:7pt;">
      ...
      <tr>
        <th class="IRS1118ScheduleI_GenericCell" style="width:6mm;height:10mm;background-color:lightgrey;text-align:center;" rowspan="2" scope="col">&nbsp;</th>
	...
	<xsl:choose>
	   ...
	   <xsl:otherwise>
        ...
        <xsl:call-template name="PopulateAmount">
          <xsl:with-param name="TargetNode" select="GrossFrgnOilGasExtractionIncm"/>
        </xsl:call-template>  
        </td>
        <td class="IRS1118ScheduleI_GenericCell">
        <xsl:call-template name="PopulateAmount">
          <xsl:with-param name="TargetNode" select="GrossForeignOilRelatedIncome"/>
        </xsl:call-template>  
        </td>
        <td class="IRS1118ScheduleI_GenericCell">
        <xsl:call-template name="PopulateAmount">
      ...
      <tr>
        ...
        <xsl:call-template name="PopulateAmount">
          <xsl:with-param name="TargetNode" select="$FormData/TotalsFrgnOilGasExtIncome/GrossFrgnOilGasExtractionIncm"/>
        </xsl:call-template>  
        </td>
        <td class="IRS1118ScheduleI_GenericCell" style="height:4mm;">
        <xsl:call-template name="PopulateAmount">
          <xsl:with-param name="TargetNode" select="$FormData/TotalsFrgnOilGasExtIncome/GrossForeignOilRelatedIncome"/>
        </xsl:call-template>  
        </td>
        <td class="IRS1118ScheduleI_GenericCell" style="height:4mm;">
        <xsl:call-template name="PopulateAmount">
  ...
  <div class="LineContainer">
   ...
   <div class="LineContainer">
      ...
      <xsl:call-template name="PopulateAmount">
        <xsl:with-param name="TargetNode" select="$FormData/CombinedForeignOilAndGasIncome"/>
      </xsl:call-template>  
      </div>
  </div>
   <!-- spacer line-->
  ...
  <div class="LineContainer">
   ...
   <div class="LineContainer">
      ...
      <div class="IRS1118ScheduleI_MoneyField">
      <xsl:call-template name="PopulateAmount">
        <xsl:with-param name="TargetNode" select="$FormData/Section907aExcessLimitation"/>
      </xsl:call-template>  
      </div>        
      </div>
  </div>
   ...
   <div class="LineContainer">
      ...
      <div class="IRS1118ScheduleI_LineDesc">schedule showing the computation in detail
        <xsl:call-template name="SetFormLinkInline">
           <xsl:with-param name="TargetNode" select="$FormData/EnterTheSumOfAnyCarryover"/>
       </xsl:call-template>
             <span class="DotSpacing">.........................</span>  
      </div>
      <div class="IRS1118ScheduleI_MoneyField">      
   ...
   <div class="LineContainer">
      ...
      <div class="IRS1118ScheduleI_MoneyField" style="border-bottom-width:1px;">
      <xsl:call-template name="PopulateAmount">
        <xsl:with-param name="TargetNode" select="$FormData/SmallerOfExcLmtAndSumCarryover"/>
      </xsl:call-template>  
      </div>
  </div>
