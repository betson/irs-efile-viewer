...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template name="OthClStockMeetRegTradTestSchTemp">
    <table id="GeneralDependencyTbl" class="styDepTblLandscape" style="font-size: 8pt" cellspacing="0">
      ...
      <tfoot/>
      <tbody>
        <xsl:for-each select="$DependencyData/OtherClassOfStockInfo">
          <tr>
            ...
            <td class="styDepTblCell" style="width:59mm;text-align:left;vertical-align:middle;font-size:7pt">
              ...
              <xsl:attribute name="rowspan"><xsl:value-of select="count(TotalPctValueSharesOwned)"/></xsl:attribute>
            ...
            <td class="styDepTblCell" style="width:22mm;text-align:center;vertical-align:middle;font-size:7pt">
              ...
              <xsl:attribute name="rowspan"><xsl:value-of select="count(TotalPctValueSharesOwned)"/></xsl:attribute>
              <xsl:call-template name="PopulateText">
                <xsl:with-param name="TargetNode" select="DaysStockCloselyHeld"/>
              </xsl:call-template>
              <span class="styTablecellpad"/>
            </td>
            <!--Column 3 - Total Percentage Of The Vote And Value Of The Class of Stock That Is Owned by 5% Shareholders -->
            <td class="styDepTblCell" style="width:32mm;text-align:center;vertical-align:middle;font-size:7pt">
              <!--Allow row to expand by column -->
              <xsl:attribute name="rowspan"><xsl:value-of select="count(TotalPctValueSharesOwned)"/></xsl:attribute>
              <xsl:call-template name="PopulateText">
                <xsl:with-param name="TargetNode" select="TotalPctAndValueStockOwned"/>
              </xsl:call-template>
              <span class="styTablecellpad"/>
            </td>
            ...
            <td class="styDepTblCell" style="width:32mm;text-align:center;vertical-align:middle;font-size:7pt">
              ...
              <xsl:attribute name="rowspan"><xsl:value-of select="count(TotalPctValueSharesOwned)"/></xsl:attribute>
              <xsl:call-template name="PopulateText">
                <xsl:with-param name="TargetNode" select="TotalQualifiedShareholders"/>
              </xsl:call-template>
              <span class="styTablecellpad"/>
            </td>
            <!--TEST 1-->
            <xsl:if test="count(TotalPctValueSharesOwned)=0 or count(TotalPctValueSharesOwned)=1">
              ...
              <td class="styDepTblCell" style="width:16mm;text-align:center;font-size:7pt">
                <xsl:call-template name="PopulateText">
                  <xsl:with-param name="TargetNode" select="TotalPctValueSharesOwned/CountryCode"/>
                </xsl:call-template>
                <span class="styTablecellpad"/>
              </td>
              <!--Column 6 - Percentage -->
              <td class="styDepTblCell" style="width:36mm;text-align:center;font-size:7pt">
                <xsl:call-template name="PopulateText">
                  <xsl:with-param name="TargetNode" select="TotalPctValueSharesOwned/Percentage"/>
                </xsl:call-template>
                <span class="styTablecellpad"/>
              </td>
            ...
            <!--TEST 2-->
            <xsl:if test="count(TotalPctValueSharesOwned)&gt;1">
              <!--Begin repeated within row for Column 5 and 6 -->
              <!--Column 5 - Country Code -->
              <td class="styDepTblCell" style="width:16mm;text-align:center;font-size:7pt">
                <xsl:call-template name="PopulateText">
            ...
            <td class="styDepTblCell" style="width:22mm;text-align:center;vertical-align:middle;font-size:7pt">
              ...
              <xsl:attribute name="rowspan"><xsl:value-of select="count(TotalPctValueSharesOwned)"/></xsl:attribute>
              <xsl:call-template name="PopulateText">
                <xsl:with-param name="TargetNode" select="DaysShareholdersOwnedStock"/>
              </xsl:call-template>
              <span class="styTablecellpad"/>
            </td>
          </tr>
          <xsl:for-each select="TotalPctValueSharesOwned">
