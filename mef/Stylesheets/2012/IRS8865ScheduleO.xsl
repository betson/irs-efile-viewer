...
<!--						<div class="styForm8865ScheduleOFN" style="width:28mm;height:17.25mm;float:left;">  -->
 ...
 <!--         Attach to Form 8865. See Instructions for Form 8865. -->
			   Information about Schedule O (Form 8865) and its separate instructions is at <i>www.irs.gov/form8865</i>
						</div>               
						</div>
        ...
        Filer's identifying number<br/>
					...
					<div class="styBB" style="width:187mm;">
						<div class="styNameBox" style="width:98mm;height:8mm;padding-bottom:1.4mm;">
        Name of foreign partnership<br/>
						...
						<div class="styFNBox" style="float:left; padding-left:2mm;width:26mm; height:8mm;font-size:7pt;font-family:verdana;"><b>
           EIN (if any) </b><br/><br/>
 								<xsl:choose>
									...
									<xsl:otherwise>
										<xsl:call-template name="PopulateText">
											<xsl:with-param name="TargetNode" select="$Form8865ScheduleOData/MissingEINReason"/>
										</xsl:call-template>
									</xsl:otherwise>
								</xsl:choose>          
							</div>
						<div style="float:left; padding-left:2mm;font-size:7pt;font-family:verdana;height:8mm;"><b>          
           Reference ID number (see instructions)  </b> <br/><br/>
 									<xsl:call-template name="PopulateText">
					...
					<xsl:variable name="p1RecCount1" select="count($Form8865ScheduleOData/StkNtsRcvblPyblOtherSec)"/>
					...
					<xsl:variable name="MSCount">
						<xsl:choose>
							<xsl:when test="count($Form8865ScheduleOData/StkNtsRcvblPyblOtherSec) &lt; 5">
								<xsl:number value="4"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="count($Form8865ScheduleOData/StkNtsRcvblPyblOtherSec)"/>
							</xsl:otherwise>
						</xsl:choose>
					</xsl:variable>
      ...
      <span style="width:36px;"/>
						<span class="styBoldText">Schedule O (Form 8865) 2012</span>
					</div>
					<br/>
					<br class="pageEnd"/>
	...
	<xsl:template name="p1Template1">
		...
		<xsl:if test="$index &lt;= $max">
			<tr>
				<xsl:if test="($Print=$Separated) and ((count($Form8865ScheduleOData/StkNtsRcvblPyblOtherSec)&gt;4) or (count($Form8865ScheduleOData/Inventory)&gt;4) or (count($Form8865ScheduleOData/TngblPropertyUsedInTradeOrBus)&gt;4) or (count($Form8865ScheduleOData/IntangibleProperty)&gt;4) or (count($Form8865ScheduleOData/OtherProperty)&gt;4))">
					<!-- Define background colors to the rows -->
          ...
          Stock, notes receivable and payable, and other <br/>securities  
				...
				<td class="styForm8865ScheduleOTablesCells" style="text-align:center">
					<xsl:call-template name="PopulateText">
						<xsl:with-param name="TargetNode" select="$Form8865ScheduleOData/StkNtsRcvblPyblOtherSec[$index]/DateOfTransfer"/>
					</xsl:call-template>
				</td>
				<!-- **************** this is where the line starts  ***************** -->
				<td class="styForm8865ScheduleOTablesCells">
					<xsl:call-template name="PopulateText">
						<xsl:with-param name="TargetNode" select="$Form8865ScheduleOData/StkNtsRcvblPyblOtherSec[$index]/NumberOfItemsTransferred"/>
						<!--   <xsl:with-param name="MaxSize" select="10" />   -->
					</xsl:call-template>
				</td>
				<td class="styForm8865ScheduleOTablesCells">
					<xsl:call-template name="PopulateAmount">
						<xsl:with-param name="TargetNode" select="$Form8865ScheduleOData/StkNtsRcvblPyblOtherSec[$index]/FairMarketValueOnDateOfTrnsfr"/>
					</xsl:call-template>
				</td>
				<td class="styForm8865ScheduleOTablesCells">
					<xsl:call-template name="PopulateAmount">
						<xsl:with-param name="TargetNode" select="$Form8865ScheduleOData/StkNtsRcvblPyblOtherSec[$index]/CostOrOtherBasis"/>
					</xsl:call-template>
				</td>
				<td class="styForm8865ScheduleOTablesCells" style="text-align:left">
					<xsl:call-template name="PopulateText">
						<xsl:with-param name="TargetNode" select="$Form8865ScheduleOData/StkNtsRcvblPyblOtherSec[$index]/AllocationMethod"/>
					</xsl:call-template>
				</td>
				<td class="styForm8865ScheduleOTablesCells">
					<xsl:call-template name="PopulateAmount">
						<xsl:with-param name="TargetNode" select="$Form8865ScheduleOData/StkNtsRcvblPyblOtherSec[$index]/GainRecognizedOnTransfer"/>
					</xsl:call-template>
				</td>
				<td class="styForm8865ScheduleOTablesCells" style="border-right:0">
					<xsl:call-template name="PopulatePercent">
						<xsl:with-param name="TargetNode" select="$Form8865ScheduleOData/StkNtsRcvblPyblOtherSec[$index]/PctIntInPartnershipAfterTrnsfr"/>
					</xsl:call-template>
				</td>
			</tr>
			<xsl:call-template name="p1Template1">
