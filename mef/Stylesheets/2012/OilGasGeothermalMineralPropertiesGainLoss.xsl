...
<!-- Last Modified by James Ganzy on 08/28/2012 -->
		  ...
		  <head>
			...
			<body class="styBodyClass">
				...
				<table id="ILSTbl" class="styDepTblLandscape" style="font-size: 8pt">
				  ...
				  <tfoot/>
					<tbody>
						<xsl:for-each select="$DependencyData/GeothermalPropertiesGroup">
						  <!-- set row background -->
						  <tr>
							...
							<td class="stydepTblCell" style="width:30mm; text-align:right;padding-top:3mm;">
							  <xsl:call-template name="PopulateAmount">
								<xsl:with-param name="TargetNode" select="ShareOfAmtRealizedFromDispos"/>
							  </xsl:call-template>
							</td>
							<!-- Partner's Share of the Partnership's Adjusted Basis in the Property - Column 3 -->
							<td class="stydepTblCell" style="width:30mm; text-align:right;padding-top:3mm;">
							  <xsl:call-template name="PopulateAmount">
								<xsl:with-param name="TargetNode" select="ShareOfPartnershipsAdjBasis"/>
							  </xsl:call-template>
							</td>
							<!-- Partner's Share of the Total Intangible Drilling Cost - Column 4 -->
							<td class="stydepTblCell" style="width:30mm; text-align:right;padding-top:3mm">
							  <xsl:call-template name="PopulateAmount">
								<xsl:with-param name="TargetNode" select="ShareOfTotIntagibleDrllngCosts"/>
							  </xsl:call-template>
							</td>
							<!-- Partner's Share of the Development Cost - Column 5 -->
							<td class="stydepTblCell" style="width:30mm; text-align:right;padding-top:3mm">
							  <xsl:call-template name="PopulateAmount">
								<xsl:with-param name="TargetNode" select="ShareOfTotalDevelopmentCosts"/>
							  </xsl:call-template>
							</td>
							<!-- Partner's Share of the Total Mining Exploration Costs (Section 59(e) Expenditures - Column 6 -->
							<td class="stydepTblCell" style="width:30mm; text-align:right;padding-top:3mm">
							  <xsl:call-template name="PopulateAmount">
								<xsl:with-param name="TargetNode" select="ShareOfTotMiningExplortionCost"/>
							  </xsl:call-template>
							</td>
						  </tr>
						</xsl:for-each>
