            ...
            <div >(Rev. October 2005)</div>
					...
					<div style="width:187mm;">
						...
						<span style="border-bottom:1px dashed black;width:30mm;">
							<xsl:call-template name="PopulateText">
								<xsl:with-param name="TargetNode" select="$Form2120Data/CalendarYear" />
							</xsl:call-template>
						...
						<div style="width:187mm;border-bottom:1px dashed black;"><span style="line-height:8mm;">
							<xsl:call-template name="PopulateText">
								<xsl:with-param name="TargetNode" select="$Form2120Data/QualifyingPersonName/PersonFirstName" />
							</xsl:call-template>
							<span style="width:1mm;"/>
							<xsl:call-template name="PopulateText">
								<xsl:with-param name="TargetNode" select="$Form2120Data/QualifyingPersonName/PersonLastName" />
							</xsl:call-template>
						</span></div>
	...
	<xsl:template name="makeRow">
		...
		<div style="width:135mm;float:left;">
			<div style="width:135mm;border-bottom:1px dashed black;">
				<xsl:call-template name="PopulateText">
					<xsl:with-param name="TargetNode" select="$TargetNode/PersonFullName/PersonFirstName" />
				</xsl:call-template>
				<span style="width:4px;"/>
				<xsl:call-template name="PopulateText">
					<xsl:with-param name="TargetNode" select="$TargetNode/PersonFullName/PersonLastName" />
				</xsl:call-template>
			</div>
			Eligible person's name
		</div>
		<div style="width:35mm;float:right;text-align:center">
			<div style="width:35mm;border-bottom:1px dashed black;">
				<xsl:call-template name="PopulateSSN">
