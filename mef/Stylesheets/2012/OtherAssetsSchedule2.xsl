...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:param name="DependencyData" select="$RtnDoc/OtherAssetsSchedule2" />

  <xsl:param name="depDocTitle">
  <xsl:call-template name="PopulateDisplayName"><xsl:with-param name="TargetNode" select="$DependencyData" /></xsl:call-template>  
  </xsl:param>
  
  <xsl:template name="OtherAssetsSchedule2Temp">
    <table id="OtherAssetsSchedule2Tbl" class="styDepTbl">
  ...
  <xsl:template match="/">
    <html>
      ...
      <xsl:call-template name="DocumentHeaderDependency"></xsl:call-template>
        ...
        <xsl:call-template name="OtherAssetsSchedule2Temp" />    
      </body>
    </html>
  </xsl:template>
</xsl:stylesheet>
