...
<!--          <th class="styTblCell" scope="col">Item No.</th>       -->
      ...
      <xsl:for-each select="$DependencyData/DeMinimisException">
        <tr>
...
<!--          <td class="styDepTblCell" style="text-align:center;" >
            ...
            <xsl:choose>
              ...
              <xsl:otherwise>
                <xsl:call-template name="PopulateSSN">
                  <xsl:with-param name="TargetNode" select="PersonSSN" />
                </xsl:call-template>
              </xsl:otherwise>
            </xsl:choose>                            
          </td>
          <td class="styDepTblCell" style="text-align:center;width:20mm" >
              <xsl:call-template name="PopulateText">
                <xsl:with-param name="TargetNode" select="EffectiveDateOfElection" />
              </xsl:call-template>
          </td>
          <td class="styDepTblCell" style="text-align:left;" >
              <xsl:call-template name="PopulateText">
                <xsl:with-param name="TargetNode" select="TradeOrBusinessType" />
              </xsl:call-template>
          </td>      
        </tr>
      </xsl:for-each>
