...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:param name="CCCLDCMSData" select="$RtnDoc/CCCLoanDetailCashMethodStmt"/>
	<!-- Template to display CCC Loan Detail Cash Method Statement-->
	<xsl:template name="ShowCCCLDCMS">
		<table id="CCCLDCMSTbl" class="styDepTbl" style="font-size:7pt;width:187mm;table-layout:fixed;">
			...
			<tfoot/>
			<tbody>
				<xsl:for-each select="$CCCLDCMSData/CCCLoanDetail">
					<tr>
	...
	<xsl:param name="depDocTitle">
		<xsl:call-template name="PopulateDisplayName">
			<xsl:with-param name="TargetNode" select="$CCCLDCMSData"/>
		</xsl:call-template>
	</xsl:param>
	<!-- Main template -->
	<xsl:template match="/">
		<html>
			...
			<body class="styBodyClass">
				...
				<xsl:call-template name="ShowCCCLDCMS"/>
				<br/>
			</body>
		</html>
	</xsl:template>
