...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template name="PopulateForeignAddressInline">
		...
		<xsl:variable name="addressLine2" select="$TargetNode/AddressLine2"/>
		<span>
		...
		<xsl:if test="$TargetNode/City != '' or $TargetNode/ProvinceOrState != '' or $TargetNode/PostalCode != ''">
			...
			<xsl:if test="$TargetNode/City != '' and $TargetNode/ProvinceOrState != ''">,<span style="width:2px;"/>
			</xsl:if>
			<xsl:call-template name="PopulateText">
				<xsl:with-param name="TargetNode" select="$TargetNode/ProvinceOrState"/>
			</xsl:call-template>
			<span style="width:7px;"/>
			<xsl:call-template name="PopulateText">
				<xsl:with-param name="TargetNode" select="$TargetNode/PostalCode"/>
			</xsl:call-template>
		</xsl:if>
		<xsl:value-of select="$Delimiter"/>
		<span>
	...
	<xsl:template name="PopulateUSAddressInline">
		...
		<xsl:if test="$TargetNode/City != '' or $TargetNode/State != '' or $TargetNode/ZIPCode != ''">
			...
			<!-- Only display a comma between city and state if both city and state exist -->
			<xsl:if test="$TargetNode/City != '' and $TargetNode/State != ''">,<span style="width:2px;"/>
			</xsl:if>
			<xsl:call-template name="PopulateText">
