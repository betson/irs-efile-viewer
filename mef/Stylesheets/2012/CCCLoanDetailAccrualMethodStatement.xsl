...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:param name="CCCLDAMSData" select="$RtnDoc/CCCLoanDetailAccrualMethodStmt"/>
	<!-- Template to display Commodity Credit Corporation Loan Detail Accrual Method Statement-->
	<xsl:template name="ShowCCCLDAMS">
		<table id="CCCLDAMSTbl" class="styDepTbl" style="font-size:7pt;width:187mm;table-layout:fixed;">
			...
			<tfoot/>
			<tbody>
				<xsl:for-each select="$CCCLDAMSData/CCCLoanDetail">
					<tr>
	...
	<xsl:param name="depDocTitle">
		<xsl:call-template name="PopulateDisplayName">
			<xsl:with-param name="TargetNode" select="$CCCLDAMSData"/>
		</xsl:call-template>
	</xsl:param>
	<!-- Main template -->
	<xsl:template match="/">
		<html>
			...
			<body class="styBodyClass">
				...
				<xsl:call-template name="ShowCCCLDAMS"/>
				<br/>
			</body>
		</html>
	</xsl:template>
