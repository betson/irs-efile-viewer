		...
		<html lang="EN-US">
			...
			<body class="styBodyClass">
				<form id="IRS8958">
					...
					<div class="styTBB" style="width:187mm;">
						...
						<div class="styTYBox" style="width:30mm;">
							...
							<div class="stySequence" style="width:100%;height:10mm;padding-left:3mm;padding-top:1mm;">
								Attachment<br/>Sequence No. <span style="font-weight:bold;font-size:10pt;">XX</span>
							</div>
						</div>
					</div>
					<!-- Primary Taxpayer -->
						...
						<div class="styNameBox" style="width:75mm;height:10mm;">
							...
							<span style="font-size:7pt;">
								<xsl:call-template name="PopulateText">
									<xsl:with-param name="TargetNode" select="$FormData/SpouseOrPartnerFirstName"/>
								</xsl:call-template>
							</span>
						</div>
						<div class="styNameBox" style="width:74mm;height:10mm;padding-left:2px;">
							...
							<span style="font-size:7pt;">
								<xsl:call-template name="PopulateText">
									<xsl:with-param name="TargetNode" select="$FormData/SpouseOrPartnerLastName"/>
								</xsl:call-template>
							</span>
						</div>
						<div class="styEINBox" style="width:37.8mm;height:10mm;padding-left:2px;">
					...
					<!-- Line 8 table -->
					<xsl:call-template name="LineTableGroup">
						<xsl:with-param name="target" select="$FormData/RentalRealESTAllocnGrp"/>
					...
					<xsl:if test="$Print = $Separated">
						...
						<xsl:if test="count($FormData/RentalRealESTAllocnGrp) &gt; 3">
							<xsl:call-template name="SeparatedTableGroup">
								<xsl:with-param name="target" select="$FormData/RentalRealESTAllocnGrp"/>
								<xsl:with-param name="number" select="8"/>
								<xsl:with-param name="description">Rents, Royalties, Partnerships, Estates, Trusts</xsl:with-param>
		...
		<table class="styDepTbl" cellspacing="0" style="font-size:7pt;width:187mm;">
			<thead class="styTableThead">
				<tr class="styDepTblHdr">
					...
					<th class="styDepTblCell" scope="col" style="width:37mm;">1<br/>Total Income<br/>(Community/Separate)</th>
					<th class="styDepTblCell" scope="col" style="width:37mm;">
						2<br/>Allocated to Spouse, RDP, or California Same-Sex Spouse<br/>SSN 
						<xsl:call-template name="PopulateReturnHeaderFiler">
							<xsl:with-param name="TargetNode">PrimarySSN</xsl:with-param>
						</xsl:call-template>
					</th>
					<th class="styDepTblCell" scope="col" style="width:37mm;">
						3<br/>Allocated to Spouse, RDP, or California Same-Sex Spouse<br/>SSN 
						<xsl:call-template name="PopulateSSN">
							<xsl:with-param name="TargetNode" select="$FormData/SpouseOrPartnerSSN"/>
						</xsl:call-template>
					</th>
