...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template name="DependencyTemplate">
    ...
    <table id="CCETbl" class="styDepTbl">
      ...
      <tfoot></tfoot>
      <tbody>
        <xsl:for-each select="$DependencyData/ContractorCompExplanation">
          <tr>
            ...
            <td class="styDepTblCell" style="text-align:left">
              <xsl:if test="ContractorNamePerson != ''">
                <xsl:call-template name="PopulateText">
                  <xsl:with-param name="TargetNode" select="ContractorNamePerson" />
                </xsl:call-template>
              </xsl:if>
              <xsl:if test="ContractorNameBusiness/BusinessNameLine1 != ''">
                <xsl:call-template name="PopulateText">
                  <xsl:with-param name="TargetNode" select="ContractorNameBusiness/BusinessNameLine1" />
                </xsl:call-template>
              </xsl:if>
              <xsl:if test="ContractorNameBusiness/BusinessNameLine2 != ''"><br/>
                <xsl:call-template name="PopulateText">
                  <xsl:with-param name="TargetNode" select="ContractorNameBusiness/BusinessNameLine2" />
                </xsl:call-template>
              </xsl:if>
            </td>
            <td class="styDepTblCell" style="text-align:left;">
