...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:param name="DependencyData" select="$RtnDoc/FiduciaryDistributionSchedule"/>
  <xsl:param name="depDocTitle">
    <xsl:call-template name="PopulateDisplayName">
      <xsl:with-param name="TargetNode" select="$DependencyData"/>
		  <!-- - Form 1120F Displays as - Fiduciary Distribution Schedules - --> 
    </xsl:call-template>
  </xsl:param>
  <xsl:template name="FiduciaryDistributionScheduleTemp">  
  ...
  <xsl:template match="/">
    <xsl:text disable-output-escaping="yes">&lt;!DOCTYPE html&gt;</xsl:text>
      ...
      <body class="styBodyClass">
        ...
        <xsl:call-template name="FiduciaryDistributionScheduleTemp"/>
      </body>
    </html>
  </xsl:template>
</xsl:stylesheet>