...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:param name="BDDSData" select="$RtnDoc/BadDebtDeductionStatement"/>
  
  <!-- Template to display Bad Debt Deduction Statement -->
  <xsl:template name="ShowBDDS">    
    <table id="BDDSTbl" class="styDepTbl">  
      <thead class="styTableThead">
       ...
       <tbody>    
        <xsl:for-each select="$BDDSData/BadDebtDeductionGrp">
          <tr>
  ...
  <xsl:template name="ShowBDDS">
    ...
    <xsl:for-each select="$BDDSData/BadDebtDeduction">
      <table id="BDDSTbl" class="styDepTbl">          
...
-->  
  ...
  <xsl:param name="depDocTitle">
    <xsl:call-template name="PopulateDisplayName"><xsl:with-param name="TargetNode" select="$BDDSData"/></xsl:call-template>  
  </xsl:param>
  <!-- Main template -->
  <xsl:template match="/">
    <xsl:text disable-output-escaping="yes">&lt;!DOCTYPE html&gt;</xsl:text>
      ...
      <body class="styBodyClass">
        ...
        <xsl:call-template name="PopulateDepCommonLeftover"><xsl:with-param name="TargetNode" select="$BDDSData"/></xsl:call-template>        
                
        <xsl:call-template name="ShowBDDS"/>          
        <br/>                      
      </body>
    </html>
  </xsl:template>
