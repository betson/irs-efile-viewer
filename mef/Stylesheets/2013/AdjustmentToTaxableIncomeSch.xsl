    ...
    <table id="AdTTI" class="styDepTbl" style="font-size:7pt;">
      ...
      <xsl:for-each select="$DependencyData/NetAdjustmentsInfo">
        <tr>
          ...
          <td class="styDepTblCell" style="width:67mm;text-align:center;">            
            <xsl:call-template name="PopulateText"><xsl:with-param name="TargetNode" select="Yr"/></xsl:call-template>             
          </td>                      
          <td class="styDepTblCell" style="width:120mm;text-align:right;">
            <xsl:call-template name="PopulateAmount"><xsl:with-param name="TargetNode" select="NetOfAllPropAndOtherAdjForPY"/>
            </xsl:call-template>             
          </td>
        </tr>
      </xsl:for-each>
