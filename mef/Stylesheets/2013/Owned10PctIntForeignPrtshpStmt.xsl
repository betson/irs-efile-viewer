...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<!--  - Common Form to display as - Owned 10% interest in foreign partnership statement- -->
		...
		<table id="OPFPTbl" class="styDepTblLandscape" style="margin-top:4mm;">
			<thead class="styTableThead">
				<tr class="styDepTblHdr">
					...
					<th class="styDepTblCell" scope="col" rowspan="2" style="width:22mm;">Entity EIN<br/>(if any)</th>
			...
			<tbody>
				<xsl:for-each select="$DependencyData/Owned10PctForeignPartnership">
					<tr>
						...
						<td class="styDepTblCell" style="text-align:center;witth:22mm">
							<xsl:if test="EIN">
						...
						<td class="styDepTblCell" align="left" valign="top">
							<xsl:call-template name="PopulateText">
								<xsl:with-param name="TargetNode" select="FormsFiledTxt"/>
							</xsl:call-template>
						</td>
						<td class="styDepTblCell" align="left" valign="top" style="width:39mm;">
							<xsl:call-template name="PopulateText">
								<xsl:with-param name="TargetNode" select="TaxMatterPartnerName/BusinessNameLine1"/>
							</xsl:call-template>
							<xsl:if test="normalize-space(TaxMatterPartnerName/BusinessNameLine2) != ''">
								<br/>
								<xsl:call-template name="PopulateText">
									<xsl:with-param name="TargetNode" select="TaxMatterPartnerName/BusinessNameLine2"/>
								</xsl:call-template>
							</xsl:if>
						</td>
