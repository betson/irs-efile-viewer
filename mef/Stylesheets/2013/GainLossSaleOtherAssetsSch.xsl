					...
					<tr>
						...
						<td class="styDepTblCell" align="left" valign="top">
							<xsl:call-template name="PopulateText">
								<xsl:with-param name="TargetNode" select="PurchaserNameGrp/PersonNm"/>
							</xsl:call-template> 
							<xsl:if test="normalize-space(PurchaserNameGrp/BusinessName/BusinessNameLine1) != ''">
								<br/>
								<xsl:call-template name="PopulateText">
									<xsl:with-param name="TargetNode" select="PurchaserNameGrp/BusinessName/BusinessNameLine1"/>
								</xsl:call-template> 
							</xsl:if>
							<xsl:if test="normalize-space(PurchaserNameGrp/BusinessName/BusinessNameLine2) != ''">
								<br/>
								<xsl:call-template name="PopulateText">
									<xsl:with-param name="TargetNode" select="PurchaserNameGrp/BusinessName/BusinessNameLine2"/>
								</xsl:call-template> 
							</xsl:if>
						</td>
		...
		<html>
			...
			<body class="styBodyClass">  
				...
				<div class="styDepTitleLineLandscape">
					<div class="styDepTitle" style="width:251.8mm">
						<xsl:value-of select="$depDocTitle"/>            
					</div>
				</div>
