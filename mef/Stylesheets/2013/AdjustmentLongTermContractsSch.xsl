...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template name="ShowAdjustmentLongTermContractsSch">
    ...
    <table id="AdjustmentLongTermContractsSchTbl" class="styDepTblLandscape">
      ...
      <tfoot/>
      <tbody>
        <xsl:for-each select="$AdjustmentLongTermContractsSchData/AdjustmentLongTermContractsGrp">  
          <tr>
            ...
            <xsl:choose>
				<xsl:when test="count (PYLongTermContractGrp) &gt; 1">
					<td class="styDepTblCell" scope="col" style="text-align:left;font-size: 7pt; width: 35mm;border-bottom:0px;">
					  <xsl:call-template name="PopulateText">
						<xsl:with-param name="TargetNode" select="ContractNum"/>
					  </xsl:call-template>
					</td>
					<td class="styDepTblCell" scope="col" style="text-align:left;font-size: 7pt;border-bottom:0px;">
					  <xsl:call-template name="PopulateText">
            ...
            <xsl:if test="count(PYLongTermContractGrp)=0 or count (PYLongTermContractGrp)=1">
              <td class="styDepTblCell" scope="col" style="text-align:center;font-size: 7pt; width: 15mm;">
                <xsl:call-template name="PopulateText">
                  <xsl:with-param name="TargetNode" select="PYLongTermContractGrp/Yr"/>
                </xsl:call-template>
              </td>
              <td class="styDepTblCell" scope="col" style="text-align:right;font-size: 7pt; width: 30mm;">
                <xsl:call-template name="PopulateAmount">
                  <xsl:with-param name="TargetNode" select="PYLongTermContractGrp/PYScheduleAdjustmentsGrp/PYIncmPrevRptBsdOnEstCntrctAmt"/>
                </xsl:call-template>
              </td>
              <td class="styDepTblCell" scope="col" style="text-align:right;font-size: 7pt; width: 30mm;">
                <xsl:call-template name="PopulateAmount">
                  <xsl:with-param name="TargetNode" select="PYLongTermContractGrp/PYScheduleAdjustmentsGrp/PYIncmAllocBsdOnActlCntrctAmt"/>
                </xsl:call-template>
              </td>
              <td class="styDepTblCell" scope="col" style="text-align:right;font-size: 7pt; width: 30mm;">
                <xsl:call-template name="PopulateAmount">
                  <xsl:with-param name="TargetNode" select="PYLongTermContractGrp/PYScheduleAdjustmentsGrp/PYOtherAdjustmentsAmt"/>
                </xsl:call-template>
              </td>
              <td class="styDepTblCell" scope="col" style="text-align:right;font-size: 7pt; width: 30mm;">
                <xsl:call-template name="PopulateAmount">
                  <xsl:with-param name="TargetNode" select="PYLongTermContractGrp/PYScheduleAdjustmentsGrp/PYNetAdjToIncmAmt"/>
                </xsl:call-template>
              </td>
            </xsl:if>
            <!-- If there is more than one set of data, show the first set with Row 1 background -->
            <xsl:if test="count(PYLongTermContractGrp)&gt;1">              
          ...
          <xsl:for-each select="PYLongTermContractGrp">        
            <xsl:if test="position() &gt; 1">
              <tr>
               ...
               <!--empty for first three columns, if there are more than one records-->

				<td class="styDepTblCell" style="text-align:left;font-size: 7pt; width: 35mm;border-right:1px;border-color:black;border-right-style:solid;">
				   <xsl:attribute name="class">
					  <xsl:choose>
						 <xsl:when test="(count(../preceding-sibling::*)+1) mod 2 = 1">styDepTblRow1</xsl:when>
						 <xsl:otherwise>styDepTblRow2</xsl:otherwise>
					  </xsl:choose>
					</xsl:attribute>
				</td>
				<td class="styDepTblCell"  style="text-align:left;font-size: 7pt;border-right:1px;border-color:black;border-right-style:solid;">
				   <xsl:attribute name="class">
					  <xsl:choose>
						 <xsl:when test="(count(../preceding-sibling::*)+1) mod 2 = 1">styDepTblRow1</xsl:when>
						 <xsl:otherwise>styDepTblRow2</xsl:otherwise>
                ...
                <td class="styDepTblCell" scope="col" style="text-align:right;font-size: 7pt; width: 30mm">
                  <xsl:call-template name="PopulateAmount">
                    <xsl:with-param name="TargetNode" select="PYScheduleAdjustmentsGrp/PYIncmPrevRptBsdOnEstCntrctAmt"/>
                  </xsl:call-template>
                </td>
                <td class="styDepTblCell" scope="col" style="text-align:right;font-size: 7pt; width: 30mm">
                  <xsl:call-template name="PopulateAmount">
                    <xsl:with-param name="TargetNode" select="PYScheduleAdjustmentsGrp/PYIncmAllocBsdOnActlCntrctAmt"/>
                  </xsl:call-template>
                </td>
                <td class="styDepTblCell" scope="col" style="text-align:right;font-size: 7pt; width: 30mm">
                  <xsl:call-template name="PopulateAmount">
                    <xsl:with-param name="TargetNode" select="PYScheduleAdjustmentsGrp/PYOtherAdjustmentsAmt"/>
                  </xsl:call-template>
                </td>
                <td class="styDepTblCell" scope="col" style="text-align:right;font-size: 7pt; width: 30mm">
                  <xsl:call-template name="PopulateAmount">
                    <xsl:with-param name="TargetNode" select="PYScheduleAdjustmentsGrp/PYNetAdjToIncmAmt"/>
                  </xsl:call-template>
                </td>
              </tr>
            </xsl:if>
