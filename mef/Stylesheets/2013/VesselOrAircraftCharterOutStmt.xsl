...
<td class="styDepTblRow1Cell" scope="col" style="width:60mm;text-align:left;font-size: 7pt">
	<!--Choice of Business Name versus Name -->
								<xsl:choose>
									<xsl:when test="LessorBusinessName/BusinessNameLine1 !=''">
										<xsl:call-template name="PopulateText">
											<xsl:with-param name="TargetNode" select="LessorBusinessName/BusinessNameLine1"/>
										</xsl:call-template>
										<xsl:if test="LessorBusinessName/BusinessNameLine2 !=''">
											<br/>
											<xsl:call-template name="PopulateText">
												<xsl:with-param name="TargetNode" select="LessorBusinessName/BusinessNameLine2"/>
											</xsl:call-template>
										</xsl:if>
									</xsl:when>
									<xsl:otherwise>
										<xsl:call-template name="PopulateText">
											<xsl:with-param name="TargetNode" select="LessorPersonNm"/>
										</xsl:call-template>
									</xsl:otherwise>
								</xsl:choose>
