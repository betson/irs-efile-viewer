...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"> 
 ...
 <xsl:output method="html" indent="yes"/>
  ...
  <xsl:param name="DependencyData" select="$RtnDoc/OthUSDisregEntSch"/>
  <xsl:param name="depDocTitle">
  <xsl:call-template name="PopulateDisplayName">
   <xsl:with-param name="TargetNode" select="$DependencyData"/>
   </xsl:call-template>  
   </xsl:param>
  <!-- Template to display OtherUSDisregardedEntitySchedule -->
  
  <xsl:template name="OtherUSDisregardedEntitySchedule">
    <!-- OtherUSDisregardedEntitySchedule -->
    <table class="styDepTbl">
      ...
      <thead class="styTableThead">
        ...
        <tfoot/>
       <tbody>
        <xsl:for-each select="$DependencyData/PrtshpOthUSDisregardedEntGrp">
          <tr>
  ...
  <xsl:template match="/">
    <html>
      ...
      <body class="styBodyClass">      
        ...
        <xsl:call-template name="OtherUSDisregardedEntitySchedule"/>          
        <br/>                      
      </body>
    </html>
  </xsl:template>
