    ...
    <table id="ATITbl" class="styDepTbl">
      ...
      <xsl:for-each select="$FormData/TaxableIncomeAdjustmentTxt">
        <tr>
          ...
          <td class="styDepTblCell" style="width:101mm; text-align:left;">
          ...
          <td class="styDepTblCell" style="text-align:right;width:36mm">
            <xsl:call-template name="PopulateAmount"><xsl:with-param name="TargetNode" select="NetChangeToDepreciationAmt"/>
            </xsl:call-template>             
          </td>                              
        </tr>
      </xsl:for-each>
