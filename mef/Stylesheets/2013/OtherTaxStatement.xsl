...
<!-- Last Modified by James Ganzy on 10/05/2010 -->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:param name="RootElt" select="$RtnDoc/OtherTaxStatement" />
  
  <!-- Template to display Other Tax Statement -->
  
  <xsl:template name="ShowData">
    <table id="ILSTbl" class="styDepTbl" style="text-align:center;">
      <thead class="styTableThead">
        <tr class="styDepTblHdr">          
           <th class="styDepTblCell" scope="col" style="width:93mm;">Other Tax Literal</th>        
           <th class="styDepTblCell" scope="col" style="width:93mm;">Other Tax Amount</th>        
        </tr>
      </thead>
       <tbody>    
        <xsl:for-each select="$RootElt/OtherTaxStmt">  
          <tr>
            ...
            <td class="styDepTblCell" style="text-align:left;" >
			  <xsl:choose>
				  <xsl:when test="OtherTaxLit">
					  <xsl:call-template name="PopulateText">
						<xsl:with-param name="TargetNode" select="OtherTaxLit" />
					  </xsl:call-template>
				  </xsl:when>
				  <xsl:otherwise>
				      <xsl:call-template name="PopulateText">
    ...
    <xsl:text disable-output-escaping="yes">&lt;!DOCTYPE html&gt;</xsl:text>
      ...
      <body class="styBodyClass" >
        ...
        <div class="styDepTitleLine ">
          <span class="styDepTitle" style="width:69mm;">
