...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template name="MonthLoop">
	...
	<xsl:variable name="EINLabel">
		<xsl:choose>
			  <xsl:when test="$Language='ENGLISH'">(EIN)&lt;br/&gt;Employer identification number</xsl:when>
	...
	<xsl:variable name="AlsoCheckText">
		<xsl:choose>
			  ...
			  <xsl:otherwise>&lt;br/&gt;(Marque tambi&amp;#233;n el trimestre a la derecha).</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	...
	<xsl:variable name="InstructionsText">
		<xsl:choose>
			  ...
			  <xsl:otherwise>Use este anexo para mostrar su OBLIGACI&amp;#211;N CONTRIBUTIVA para el trimestre; NO LO USE para mostrar sus dep&amp;#243;sitos de contribuci&amp;#243;n. Cuando radique este formulario con el Formulario 941-PR no cambie su obligaci&amp;#243;n contributiva por ajustes declarados en todo Formulario 941-X (PR). Tiene que llenar este formulario y adjuntarlo al Formulario 941-PR si es depositante de itinerario bisemanal o si su obligaci&amp;#243;n contributiva acumulada en un d&amp;#237;a determinado fue $100,000 o m&amp;#225;s. Anote sus obligaciones contributivas diarias en el espacio numerado que corresponda a la fecha en la cual se pagaron los salarios. Vea el apartado 11 de la Publicaci&amp;#243;n 179 (Circular PR) Gu&amp;#237;a Contributiva Federal para Patronos Puertorrique&amp;#241;os, para m&amp;#225;s detalles.</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	...
	<xsl:variable name="TotalLiabilityText">
		<xsl:choose>
			  ...
			  <xsl:otherwise>Escriba el total de su obligaci&amp;#243;n contributiva para el trimestres (Mes 1 + Mes 2 + Mes 3)</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	...
	<xsl:variable name="FooterMiddleText">
		<xsl:choose>
    ...
    <html lang="{$HtmlLang}">
      ...
      <body class="styBodyClass">
		...
		<div style="width:187mm;height;9mm;border-bottom:2px solid black;">
				<div style="width:148mm;height:9mm;float:left;text-align:right;font-size:7pt;padding-top:5mm;padding-right:2mm;">
				<xsl:value-of disable-output-escaping="yes" select="$TotalLiabilityText" /><span style="width:5px;" /><img src="{$ImagePath}/1040_Bullet.gif" alt="Right pointing arrowhead image"/><br />
     ...
     <!-- <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< -->
          ...
          <div style="float:left;">
              <span class="styBoldText" style="font-size:8pt;"><xsl:value-of disable-output-escaping="yes" select="$FooterLeftText" /></span>                 
              
            </div>
            <div style="float:left;">
              <span style="font-size:7pt;margin-left:50px;"><xsl:value-of disable-output-escaping="yes" select="$FooterMiddleText" /></span>                 
              
            </div>
            <div style="float:right;">
              <span class="styBoldText" style="font-size:7pt;"><xsl:value-of disable-output-escaping="yes" select="$FooterRightText" /></span> (Rev. 6-2011)                      
