...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:param name="ARGLSData" select="$RtnDoc/AtRiskActySect1231GainLossStmt"/>
  
  <!-- Template to display Itemized At-Risk Activity Other Income (Loss) Statement -->
  
  <xsl:template name="ShowARGLS">
    <!-- Itemized  Schedule Data -->
    <table id="ARGLSTbl" class="styDepTbl">
      ...
      <tr class="styDepTblHdr">         
        ...
        <tfoot/>
       <tbody>
      <xsl:for-each select="$ARGLSData/AtRiskActySection1231GainLoss">
        <tr>
  ...
  <xsl:param name="depDocTitle">
    <xsl:call-template name="PopulateDisplayName"><xsl:with-param name="TargetNode" select="$ARGLSData"/></xsl:call-template>  
  </xsl:param>
  <!-- Main template -->
  <xsl:template match="/">
    <html>
      ...
      <body class="styBodyClass">
        ...
        <xsl:call-template name="PopulateDepCommonLeftover"><xsl:with-param name="TargetNode" select="$ARGLSData"/></xsl:call-template>        
                  
        <xsl:call-template name="ShowARGLS"/>          
        <br/>                      
      </body>
    </html>
  </xsl:template>
