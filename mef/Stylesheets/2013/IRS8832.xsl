		...
		<body class="styBodyClass">
            ...
            <xsl:choose> 
                  ...
                  <xsl:with-param name="TargetNode" select="$Form8832Data/ForeignAddress/City"/>
								...
								<xsl:call-template name="PopulateText">
									<xsl:with-param name="TargetNode" select="$Form8832Data/ForeignAddress/ProvinceOrState"/>
								</xsl:call-template>
								<span style="width:7px"/>                
								<xsl:call-template name="PopulateText">
									<xsl:with-param name="TargetNode" select="$Form8832Data/ForeignAddress/PostalCode"/>
								</xsl:call-template>
								<br/>                
								<xsl:call-template name="PopulateText">
									<xsl:with-param name="TargetNode" select="$Form8832Data/ForeignAddress/Country"/>
								</xsl:call-template>
							</xsl:when>         
							<xsl:otherwise> 
                <xsl:call-template name="PopulateText">
