   ...
   <table id="RecipientInformationGrpTbl" class="styDepTbl">
				...
				<tr>
					<td class="styDepTblCell" colspan="3" style="text-align:left;"><span style="float:left;padding-right:1mm;">Contribution Recipient: </span><span style="float:left;">
						<xsl:choose>
									<xsl:when test="ContributionRecipient/PersonName != ''">
										<xsl:call-template name="PopulateText">
											<xsl:with-param name="TargetNode" select="ContributionRecipient/PersonName"/>
										</xsl:call-template>
									</xsl:when>
								</xsl:choose>
								<xsl:choose>
									<xsl:when test="ContributionRecipient/BusinessName/BusinessNameLine1 != ''">
										<xsl:call-template name="PopulateText">
											<xsl:with-param name="TargetNode" select="ContributionRecipient/BusinessName/BusinessNameLine1"/>
										</xsl:call-template>
									</xsl:when>
								</xsl:choose>
								<xsl:choose>
									<xsl:when test="ContributionRecipient/BusinessName/BusinessNameLine2 != ''">
										<br/>
										<xsl:call-template name="PopulateText">
											<xsl:with-param name="TargetNode" select="ContributionRecipient/BusinessName/BusinessNameLine2"/>
										</xsl:call-template>
									</xsl:when>
								</xsl:choose>
						</span>
				...
				<tr>
					<td class="styDepTblCell" colspan="3" style="text-align:left;"><span style="float:left;padding-right:1mm;">Address: </span>
								<span style="float:left;"><xsl:if test="ContributionRecipient/AddressInUS != ''">
									<xsl:call-template name="PopulateUSAddressTemplate">
										<xsl:with-param name="TargetNode" select="ContributionRecipient/AddressInUS"/>
									</xsl:call-template>
								</xsl:if>
								<xsl:if test="ContributionRecipient/AddressInForeign != ''">									
									<xsl:call-template name="PopulateText">
										<xsl:with-param name="TargetNode" select="ContributionRecipient/AddressInForeign/AddressLine1"/>
									</xsl:call-template>
									<xsl:if test="ContributionRecipient/AddressInForeign/AddressLine1"><br />
									<xsl:call-template name="PopulateText">
										<xsl:with-param name="TargetNode" select="ContributionRecipient/AddressInForeign/AddressLine2"/>
									</xsl:call-template>
									</xsl:if><br />
									<xsl:call-template name="PopulateText">
										<xsl:with-param name="TargetNode" select="ContributionRecipient/AddressInForeign/City"/>
									</xsl:call-template>,<span style="width:2px;"/>
									<xsl:call-template name="PopulateText">
										<xsl:with-param name="TargetNode" select="ContributionRecipient/AddressInForeign/ProvinceOrState"/>
									</xsl:call-template>,<span style="width:2px;"/>
									<xsl:call-template name="PopulateText">
										<xsl:with-param name="TargetNode" select="ContributionRecipient/AddressInForeign/PostalCode"/>
									</xsl:call-template><span style="width:7px;"/>
									<xsl:call-template name="PopulateText">
										<xsl:with-param name="TargetNode" select="ContributionRecipient/AddressInForeign/Country"/>
									</xsl:call-template>
